...
#   Metodo getSQlSelectWhere        
    ...
    public static function getSQlSelectWherePracticaConNomenclador($params) {
...
//        $sql = "INSERT INTO facturacion.comprobante( cuie, id_factura, nombre_medico, 
//                   fecha_comprobante,clavebeneficiario, id_smiafiliados, 
//                   fecha_carga, comentario, marca, periodo,id_servicio,activo,
//                               id_beneficiarios, entidad_alta, idvacuna, mensaje, fila,
//                               idprestacion, idperiodo, grupo_etario, tipo_nomenclador,usuario,id_nomenclador_detalle)
        $sql = "INSERT INTO facturacion.comprobante( cuie, id_factura, nombre_medico, 
                   ...
                   fecha_carga, comentario, marca, periodo,id_servicio,activo,
                               id_beneficiarios)
                VALUES ('".$this->cuie."', ";
                $this->idFactura!="" ? $sql.=$this->idFactura."," : $sql.="NULL,";
                $sql .= "'".$this->nombreMedico."',";
                $this->fechaComprobante!="" ? $sql .= "'".$this->fechaComprobante."'," : $sql.="NULL,";
                $sql .= "'".$this->clavebeneficiario."',";
                $this->idSmiafiliados!="" ? $sql.=$this->idSmiafiliados."," : $sql.="NULL,";
                $this->fechaCarga!="" ? $sql .= "'".$this->fechaCarga."'," : $sql.="NULL,";
                $sql .= "'".$this->comentario."',";
                $this->marca!="" ? $sql.=$this->marca."," : $sql.="NULL,";
                $sql .= "'".$this->periodo."',";
                $this->idServicio!="" ? $sql.=$this->idServicio."," : $sql.="NULL,";
                $sql .= "'".$this->activo."',";
                $this->idBeneficiarios!="" ? $sql.=$this->idBeneficiarios."," : $sql.="NULL";
                // $sql .= "'".$this->entidad_alta."',";
                // $this->idvacuna!="" ? $sql.=$this->idvacuna."," : $sql.="NULL,";
                // $sql .= "'".$this->mensaje."',";
                // $this->fila!="" ? $sql.=$this->fila."," : $sql.="NULL,";
                // $this->idprestacion!="" ? $sql.=$this->idprestacion."," : $sql.="NULL,";
                // $this->idperiodo!="" ? $sql.=$this->idperiodo."," : $sql.="NULL,";
                // $sql .= "'".$this->grupoEtario."',";
                // $sql .= "'".$this->tipoNomenclador."',";
                // $this->usuario!="" ? $sql.=$this->usuario."," : $sql.="NULL,";
                // $this->idNomencladorDetalle!="" ? $sql.=$this->idNomencladorDetalle : $sql.="NULL";
                $sql .= ")                     
    ...
    /* Crea un nuevo comprobante con los mismos
       datos que el original, a excepcion del id */
    public function clonarComprobante(){
//        $sql = "INSERT INTO facturacion.comprobante(cuie, id_factura, nombre_medico, fecha_comprobante,clavebeneficiario, id_smiafiliados, fecha_carga, comentario, marca, periodo,id_servicio,activo, id_beneficiarios, entidad_alta, idvacuna, mensaje, fila, idprestacion, idperiodo, grupo_etario, tipo_nomenclador,usuario,id_nomenclador_detalle) 
        $sql = "INSERT INTO facturacion.comprobante(cuie, id_factura, nombre_medico, fecha_comprobante,clavebeneficiario, id_smiafiliados, fecha_carga, comentario, marca, periodo,id_servicio,activo, id_beneficiarios) 
                SELECT cuie, id_factura, nombre_medico, fecha_comprobante,clavebeneficiario, id_smiafiliados, fecha_carga, comentario, marca, periodo,id_servicio,activo, id_beneficiarios 
                FROM facturacion.comprobante WHERE id_comprobante=".$this->getIdComprobante()." 
                RETURNING id_comprobante";
        $result = sql($sql);
        return $result->fields['id_comprobante'];
    }
...
class ComprobanteColeccion {
    ...
    public static function Filtrar($where = '', $select = '', $order = '') {
        if (strlen($where) > 0) {
            $sql = Comprobante::getSQlSelectWhere($where,$select,$order);
        } else {
    ...
    public static function existeComprobanteEnRango($fechaDesde,$fechaHasta){
        $sql = "SELECT COUNT(*) AS total 
                FROM facturacion.comprobante c
                WHERE c.fecha_comprobante BETWEEN '$fechaDesde' AND '$fechaHasta' 
               ";
        $result = sql($sql);
    ...
    public static function existeComprobanteFueraRango($idNomencDetalle,$fechaDesde="",$fechaHasta=""){
        if($fechaDesde!="" && $fechaHasta!=""){
            $where = " AND c.fecha_comprobante NOT BETWEEN '$fechaDesde' AND '$fechaHasta' ";
        }
        if($fechaDesde!="" && $fechaHasta==""){
            $where = " AND c.fecha_comprobante < '$fechaDesde' ";
        }
        if($fechaDesde=="" && $fechaHasta!=""){
            $where = " AND c.fecha_comprobante > '$fechaHasta' ";
        }
        $sql = "SELECT COUNT(*) AS total 
                FROM facturacion.comprobante c
                WHERE c.id_nomenclador_detalle=$idNomencDetalle 
                ".$where."
               ";
        $result = sql($sql);
        if($result->fields['total']>0){
    ...
    /* Devuelve el string sql que obtiene toda la 
     * info de los  comprobantes que se ajusten 
     * al criterio de busqueda suministrado via parametros
    */
    public static function getSQLInfoComprobantes($campo,$valor){
        $sql = "SELECT c.*, e.nombreefector
                FROM facturacion.comprobante c
                JOIN facturacion.smiefectores e using(cuie)
                WHERE c.".$campo."=$valor
                ORDER BY c.id_comprobante DESC";
        return $sql;
    }    
    
    public static function getSQLInfoPrestaciones($idComprobante){
        $sql = "SELECT p.*, n.codigo, n.diagnostico, n.descripcion
                FROM facturacion.prestacion p
                LEFT JOIN facturacion.nomenclador n USING (id_nomenclador)                          
                WHERE p.id_comprobante=".$idComprobante." 
                ORDER BY p.id_prestacion DESC";
        return $sql;
    }
