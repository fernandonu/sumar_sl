...
//require_once(LIB_DIR."/PHPMailerAutoload.php");
...
function enviar_mail($para,$paracc,$parabcc,$asunto,$contenido,$adjunto,$path,$htmlflag=1){
 ...
 //$mail->AltBody = encabezado_mail().$contenido.firma_mail();
 $mail->Body = encabezado_mail().$contenido.firma_mail();
 //$body=encabezado_mail().$contenido.firma_mail();
 if (!$htmlflag)$mail->IsHTML(true);
 //if (!$htmlflag)$mail->msgHTML(file_get_contents($body), dirname(__FILE__));
...
function enviar_mail_proteger($para,$paracc,$parabcc,$asunto,$contenido,$adjunto,$path,$htmlflag=1){
	$mail = new PHPMailer();
	$mail->isSMTP();
	$mail->Host = "smtp.sanluis.gov.ar"; //servidor de mi trabajo
	$mail->SMTPAuth = true; 
	$mail->Username = "protegersanluis"; 
	$mail->Password = "cvb12345";  //Cambio realizado por la autopista el dia 31-05-2019
	$mail->Timeout=50;
	$mail->SMTPSecure='tls'; 
	$mail->SMTPDebug=2;
	$mail->Debugoutput='html';
	$mail->setFrom('protegersanluis@sanluis.gov.ar', 'Programa Proteger - Ministerio de Salud');
	$mail->addAddress($para,$para);
	$mail->Subject = "Notificacion Automatica - ".$asunto;
	$mail->Body = encabezado_mail().$contenido.firma_mail();
	if (!$htmlflag)$mail->IsHTML(true);
...
/*function enviar_mail_proteger($para,$paracc,$parabcc,$asunto,$contenido,$adjunto,$path,$htmlflag=1){
 ...
 $mail->Password = "cvb12345";  //Cambio realizado por la autopista el dia 31-05-2019
 $mail->Timeout=50; 
 $mail->From = "protegersanluis@sanluis.gov.ar";
 $mail->FromName = "Programa Proteger - Ministerio de Salud";
 $mail->AddAddress($para,$para);
 $mail->Subject = "Notificacion Automatica - ".$asunto;
 $mail->Body = encabezado_mail().$contenido.firma_mail();
 if (!$htmlflag)$mail->IsHTML(true);
...
elseif (isset($parametros['restaurar_usr']))
{
...
function enviar_mail_html_proteger($para,$asunto,$contenido,$adjunto,$path,$adj=1){ 
 return enviar_mail_proteger($para,null,null,$asunto,$contenido,$adjunto,$path,'0');
}
