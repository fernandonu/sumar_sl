...
//          adjustModalMaxHeightAndPosition($(dialog.getModal()));
//        },
        closable: false,
        buttons: [{
                label: 'Aceptar',
...
function renderModal(selector, html) {
  var parent = "body",
...
$(document).ready(function() {
  ...
  $('body').on('show.bs.modal', '.modal', function() {
    // guardo la posicion actual del scroll
    modal_scroll = $(window).scrollTop();
    // muevo el scroll al principio para arreglar el error de scroll dentro del modal
    $(window).scrollTop(0);
    adjustModalMaxHeightAndPosition($(this));
  });
  $('body').on('hide.bs.modal', '.modal', function() {
    // restauro el scroll a la posicion guardada
    $(window).scrollTop(modal_scroll);
  });
});

function call_frame_func(func) {
  if ($('#frame2').length > 0 &&
      $('#frame2')[0].contentWindow) {
    eval("$('#frame2')[0].contentWindow."+func+"();");
  }
}