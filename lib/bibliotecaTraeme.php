...
function comprobarPeriodo($periodo){
    $sql="select estado from facturacion.periodo_objetivo where periodo='$periodo'";
    $res_pe = sql($sql, '') or fin_pagina();
    if ($res_pe->recordCount() > 0) {
        return $res_pe->fields['estado'];
    } else {
        return null;
    }
    
}
function consultarEstadoPeriodo($periodo){
     $sql2="select estado from facturacion.periodo_objetivo where periodo='.$periodo'";
    $res_pe = sql($sql2, '') or fin_pagina();
    if ($res_pe->recordCount() > 0) {
        return $res_pe->fields['estado'];
    } else {
        return null;
    ...
    if ($codigo['tipo'] == 'viejo') {
        ...
        $query = "SELECT id_prestacion, fecha_comprobante::date
  			...
  			INNER JOIN facturacion.nomenclador using (id_nomenclador)
                        ...
                        $queryanexo 
                        AND fecha_comprobante >= '$fecha_original'
...
function beneficiarioEmbarazadoSMI($clavebeneficiario, $periodo_comprobante, $fecha_comprobante) {
    ...
    do {
        ...
        if (($result->fields['embarazoactual'] == 'S') && ( $fecha_comprobante_time <= $fechadiagnosticoembarazo_time)) {
...
//usar el de beneficiariosSMI

function calcularGrupoEtareo($fecha_nacimiento, $fecha_comprobante, $arr_dias_anios = null) {
...
function cargaTrazadora($codigocompleto, $patologia, $grupo_etareo, $tipo_nomenclador='') {
    // Agregar tipo_nomenclador 
    $trz[0] = false;
    if ($patologia != "") {
    ...
    if ($tipo_nomenclador != "") {
        ...
        $where .= " AND tipo_nomenclador='BASICO'";
    }
    ...
    if (($res->fields['trz'] != '') or ( $res->fields ['trz'] != null )) {
        ...
        switch ($res->fields['trz']) {
            case 'LABORATORIOS':
                $trz[1] = "../trazadoras/carga_gruposanguineo.php";
                break;    
            case 'NINO':
            ...
            case 'LABORATORIOREDES':
                $trz[1] = "../remediar/carga_laboratorio.php";
                break;
            case 'TAL':
            ...
            case 'TALLER':
                ...
                break;
            case 'HOTEL':
                $trz[1] = "../trazadoras/is_trazadora_hotel.php";
                break;
            case 'PROTORT':
                $trz[1] = "../incluir/prot_ort_admin.php";
                break;
            case 'PASAJE':
                $trz[1] = "../trazadoras/is_trazadora_pasaje.php";
                break;
            case 'ANOMALIAS_CONGENITAS':
                $trz[1] = "../trazadoras/anomalia_congenita.php";
                break; 
            case 'HOMBRES_OPCION1':
                $trz[1] = "../trazadoras/hombres_form.php";
                break; 
            ...
            case 'HOMBRES_OPCION4':
                ...
                break;
            case 'SANGRE-OCULTA':
                $trz[1] = "../trazadoras/resultado_form.php";
                break;
            case 'RETINOPATIA':
                $trz[1] = "../trazadoras/carga_retinopatia.php";
                break;
            case 'HIPOACUSIA':
                $trz[1] = "../trazadoras/hipoacusia_form.php";
                break;
            case 'VDRL':
                $trz[1] = "../trazadoras/vdrl_form.php";
                break;
            case 'MAMOGRAFIA_12':
                $trz[1] = "../trazadoras/form_mamografia_bilateral.php";
                break;
            case 'ODONTOLOGIA':
                $trz[1] = "../trazadoras/form_odontologia_consulta.php";
                break;
            case 'DATO_REPORTABLE_9':
                $trz[1] = "../trazadoras/mamografia_report_9.php";
                break;
            case 'DATO_REPORTABLE_10':
                $trz[1] = "../trazadoras/mamografia_report_10.php";
                break;
            case 'DATO_REPORTABLE_11':
                $trz[1] = "../trazadoras/mamografia_report_11.php";
                break;
            case 'DATO_REPORTABLE_14':
                $trz[1] = "../trazadoras/mamografia_report_14.php";
                break;
            case 'CARDIOPATIAS_NO_CATASTROFICO':
                $trz[1] = "../trazadoras/cardiopatia/index.php";
                break;
            case 'CARDIOPATIAS_CATASTROFICO':
                $trz[1] = "../trazadoras/cardiopatia/index.php";
                break;
            case 'DOMS_01':
                $trz[1] = "../doms/index.php";
                break;
            case 'DOMS_02':
                $trz[1] = "../doms/index.php";
                break;
...
/* Metodo que corre controles sobre un comprobante y/o sus prestaciones
 * para verificar si es factible de facturarse. Existen controles 
 * exclusivos a nivel comprobante y otros a nivel prestaciones
 * Devuelve un array cuyos valores son:
 * array[0] => boolean ... indica si el comprobante se puede facturar o no
 * array[1] => text ... motivo por el cual se rechaza el comprobante/prestacion
 * array[prest_rechazadas] => array ... contiene los ids de las prestaciones del comp que se rechazan
 */
...
//            break;
        ...
        $sepuede['prest_rechazadas'] = array();
        $exluidos = controlExcluidos($cuie, $prestaciones, $grupo_etario['categoria'], $sepuede['prest_rechazadas']);
        if ($exluidos['debito']) {
            $sepuede[0] = false;
        ...
        if (!controlEmbarazadas($beneficiario, $prestaciones, $periododelcomprobante, $fecha_comprobante, $sepuede['prest_rechazadas'])) {
...
//
//Afecta CorrerControles / AgregarComprobante
function aptoPoblacionNueva($cuie, $fecha_comprobante) {
    $puede = true;

    $sql = "SELECT fechapobnueva from nacer.efe_conv ec
                WHERE cuie='$cuie'
                AND ec.activo=TRUE";

    $result = sql($sql);
    //efe_conv
    $fechapobnueva = ConvFechaComoDB($result->fields['fechapobnueva']);
...
function controlEmbarazadas($beneficiario, $prestaciones, $periodo_comprobante, $fecha_comprobante, &$prest_rechazadas) {
    ...
    while (!$prestaciones->EOF) {
        if (practicaSoloParaEmbarazadas($prestaciones->fields['id_nomenclador'])) {
            if (!BeneficiarioSmi::comprobarEmbarazo($beneficiario['clavebeneficiario'], $fecha_comprobante)) {
                $puede = false;
                if (!in_array($prestaciones->fields['id_prestacion'], $prest_rechazadas)) {
                    $prest_rechazadas[] = $prestaciones->fields['id_prestacion'];
                }
                //break;
            }
        }
...
// Determina si el idnomenclador de una o varias prestaciones en el comprobante esta en excluidos
// $prestaciones= result con join a nomenclador
//  Afecta: AgregarComprobante, Correr Controles
function controlExcluidos($cuie, $prestaciones, $etario, &$prest_rechazadas) {
    $ctrl['debito'] = false;
    $prestaciones->movefirst();
    while (!$prestaciones->EOF) {
...
// Verifica que la prestacion sea anterior a la fecha de entrada de la factura/expediente
function controlfechaPrestacion($fecha_entrada, $fecha_comprobante) {
    $fecha_entrada_aux = strtotime(ConvFechaComoDB($fecha_entrada));
    $fecha_comprobante_aux = strtotime(ConvFechaComoDB($fecha_comprobante));

    if ($fecha_entrada_aux < $fecha_comprobante_aux) {
        $ctrl['debito'] = true;
        $ctrl['msj_error'] = 'La Fecha de prestacion es mayor a la Fecha de Entrada';
        $ctrl['id_error'] = 8;
    }
    return $ctrl;
}
...
/* devuelve las prestaciones de un 
 * comprobante, que no tienen debitos
 * evaluar donde colocar el metodo (clase o aca)
 */

function prestacionesNoDebitadasEnComprobante($id_comprobante) {
    $sql = "SELECT p.* FROM facturacion.prestacion p
            LEFT JOIN facturacion.debito d ON p.id_prestacion=d.id_prestacion
            WHERE p.id_comprobante=$id_comprobante 
              AND d.id_debito IS NULL";
    $res = sql($sql, "no se pueden traer las prestaciones sin debitar") or die();
    return $res;
}
...
function cantidadDePrestaciones($id_comprobante) {
    $sql = "select count(id_prestacion) cantidad from facturacion.prestacion p
                        inner join facturacion.nomenclador using (id_nomenclador)
			where id_comprobante=$id_comprobante";
    $cant_prestaciones = sql($sql, "no se puede traer la contidad de prestaciones") or die();
...
function mandarMailSoloPerinatal($id_prestacion) {
    ...
    if (($result->fields['tipo_nomenclador'] == 'PERINATAL_CATASTROFICO' ) || ($result->fields['tipo_nomenclador'] == 'PERINATAL_NO_CATASTROFICO' )) {
        ...
        $periodo = buscarPeriodoPorId($beneficiarioenpadron['idperiodo']);
            ...
            //$para[0]= "marcefmiranda@gmail.com";
            //$para[1]= "drnorber02@gmail.com";
            //$para[2]= "bepetrella@gmail.com";          
       ...
       for($i=0; $i<count($para); $i++)
                       ...
                       $exito = enviar_mail($para[$i], null, null, $asunto, $contenido, null, null, '0');
                       }
