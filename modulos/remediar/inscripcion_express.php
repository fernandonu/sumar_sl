...
$cuie_no_hab=array();
$sql_efec_hab = "SELECT cuie
				from nacer.efe_conv 
				where conv_sumar = TRUE
				and departamento = '056'";
$res_sql = sql($sql_efec_hab) or fin_pagina();
while (!$res_sql->EOF) {
	array_push($cuie_no_hab,$res_sql->fields['cuie']);
	$res_sql->movenext();
};

$pos = array_search('D05128',$cuie_no_hab); //pedido de Guadalupe por Sandra 20-04-2023
unset($cuie_no_hab[$pos]);
...
function cambiar_patalla(){	
	...
	//si es masculino y menor de 6 años es una inscripcion con datos de padre madre o tutor
	...
	//si es menor de 6 años y femenino una inscripcion con los datos de madre padro o tutor
...
<form name='form1' action='inscripcion_express.php' method='POST'>
...
<table class="bordes" align="center" width="100%">
		 ...
		 <tr>
            ...
            /*if (es_cuie($login)) {
                $efe="SELECT * from nacer.efe_conv where cuie='$login'";
                $res_efe=sql($efe) or fin_pagina();
                $departamento=$res_efe->fields['departamento'];
                if ($departamento=='056') $disabled_b='echo disabled';
                  else $disabled_b='';
            }
            if ($login=='sandradelav' or $login=='mnolte') $disabled_b='';
            elseif ($tipo_transaccion == "B") $disabled_b='echo disabled';
            else $disabled_b='echo disabled';*/
            $disabled_b='';//codigo nuevo hablado con Sandra
			if (es_cuie($login))
				{if (in_array($login,$cuie_no_hab)) $disabled='disabled';
					else $disable='';
				}	
			else $disabled='';
		 
		 	?>
		      <input type=button name="carga_remediar_rapida" value="Clasificacion Rapida Redes" onclick="window.open('<?=$ref?>','Clasificacion','menubar=1,resizable=1,scrollbars=1,width=1000,height=750');" title="Clasificacion Rapida" <?php echo $disabled?>>&nbsp;
