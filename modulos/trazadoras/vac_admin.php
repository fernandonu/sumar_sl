...
require_once ("funciones_vacunas.php");
...
if ($_POST['guardar']=="Guardar") {   
   //$db->StartTrans();         
     ...
     if ($id_dosis_apli=='8') $id_dosis_apli='6';    
	...
	if ($resultado->RecordCount()==0) {
      ...
      //alta en el sistema de facturacion
        ...
        $edad_dias=GetCountDaysBetweenTwoDates(fecha_db($fecha_nac),date("Y-m-d"));
        $grupopoblacional=$res_2->fields['grupopoblacional'];
        $afitipocategoria=$res_2->fields['afitipocategoria'];
        $afisexo=trim($res_2->fields['afisexo']);
       ...
       $q="SELECT * from facturacion.nomenclador_detalle 
         	where '$fecha_vac' between fecha_desde and fecha_hasta
        ...
        $param_nomen=nomenclador_vacuna($id_vac_apli,$grupopoblacional,$afitipocategoria,$id_nomenclador_detalle,$afisexo);
        if ($param_nomen['id']!=-1)
          {
          
          $nomenclador=$param_nomen['id'];
          $anexo=$param_nomen['anexo'];
          //comprobante
	       ...
	       $query="INSERT into facturacion.comprobante
		      ...
		      values
		      ($id_comprobante,'$cuie','$nom_resp','$fecha_vac','$clavebeneficiario', $id_smiafiliados,'$fecha_carga','$periodo','Desde Modulo Vacunacion','1','$activo','$grupopoblacional')";	
         ...
         //prestacion
          ...
          $accion.=" - Se Genero un comprobante por ser Beneficiario de Plan SUMAR(PACES)";
          ...
          sql($log) or fin_pagina();
          $db->CompleteTrans(); 
        }//del if (nomenclador_vacuna)
      else $accion.=" - La Vacuna Ingresada no esta contemplada para ese grupo Poblacional para la facturacion";
		} else $accion.=" - Beneficiario Inactivo para facturacion";	
  } else $accion.=" - Beneficiario Externo";
	
       
  
  if ((($id_vac_apli == '5') or ($id_vac_apli == '4')) and (GetCountDaysBetweenTwoDates(fecha_db($fecha_nac),date("Y-m-d"))<744)){//cargo trz VIII
    ...
    if ((($id_vac_apli == '5') or ($id_vac_apli == '6')) and (GetCountDaysBetweenTwoDates(fecha_db($fecha_nac),date("Y-m-d"))>1825)){//cargo trz IV
			$fecha_nac_db=fecha_db($fecha_nac);
   ...
   else {$accion="Esta vacuna ya ha sido cargada";}
    ...
    {
    	$accion2="La Persona NO esta Captada por el Plan Sumar";
    }
    else
    {
    	$accion2="";
...
if ($entidad_alta=='na'){//carga de prestacion a paciente PLAN NACER
  ...
  $sexo=trim($res_comprobante->fields['afisexo']);
}
echo $html_header;
?>
<script>
...
<input type="hidden" value="<?php echo $pagina_volver?>" name="pagina_volver">
...
<input type="hidden" value="<?php echo $entidad_alta?>" name="entidad_alta"><?/*
...
<div style="display:'none';width:98%;overflow:auto;<? if ($log->RecordCount() > 3) echo 'height:60;' ?> " id="tabla_logs" >
...
<hr>
<?/*******************  FIN  LOG  ****************************/?>
<table width="85%" cellspacing=0 border=1 bordercolor=#E0E0E0 align="center" bgcolor='<?php echo $bgcolor_out?>' class="bordes">
 <tr id="mo">
    <td>
    	<?
    	if (!$id_planilla) {
        ...
        <table>
         <tr>	           
           <td align="center" colspan="2">
            <b> Número del Dato: <font size="+1" color="Red"><?php echo ($id_planilla)? $id_planilla : "Nuevo Dato"?></font> </b>
           </td>
         </tr>    
         <tr>
         	<td align="right">
            ...
            <td align='left'>
              <input type="text" size="40" value="<?php echo $apellido?>" name="apellido" <? if ($id_planilla) echo "readonly"?>>
            </td>
            ...
            <td align='left'>
              <input type="text" size="40" value="<?php echo $nombre?>" name="nombre" <? if ($id_planilla) echo "readonly"?>>
            </td>
         </tr> 
		<tr>
         	<td align="right">
            ...
            <td align='left'>
              <input type="text" size="20" value="<?php echo $dni?>" name="dni" <? if ($id_planilla) echo "readonly"?>>
            </td>
		    ...
		    <td align="left">
		    	<?$fecha_comprobante=date("d/m/Y");?>
		    	 <input type=text id=fecha_nac name=fecha_nac value='<?php echo fecha($fecha_nac);?>' size=15 readonly>
     ...
     <tr>
       <td>
        <table>
         <tr>
			...
			<td align="left">			 	
			 <select name=cuie Style="width=257px" 
				...
				>
			 <?php $user_login1=substr($_ses_user['login'],0,6);
							 ...
							 while (!$res_efectores->EOF){ 
								...
								?>
								<option value='<?php echo $cuie;?>' Style="background-color: <?php echo $color_style;?>"><?php echo $cuie." - ".$nombre_efector?></option>
								<?
			...
			<td align="right">
		    	...
		    	<?$fecha_vac=date("d/m/Y");?>
		    	 <input type=text id=fecha_vac name=fecha_vac value='<?php echo $fecha_vac;?>' size=15  readonly> 
		    	 <?php echo link_calendario("fecha_vac");?>
		    </td>
		    </td>
			<td align="right">
		    	...
		    	<?$fecha_vac_prox=date("d/m/Y");?>
		    	 <input type=text id=fecha_vac_prox name=fecha_vac_prox value='<?php echo $fecha_vac_prox;?>' size=15  readonly> 
		    	 <?php echo link_calendario("fecha_vac_prox");?>
		    </td>
         ...
         <tr>
			...
			<td align="left">			 	
			 ...
			 <?
			 $sql= "SELECT * from trazadoras.vac_apli 
					 where id_vac_apli<>29 --obsoleta exclusion del listado
					 and id_vac_apli<>30 --obsoleta exclusion del listado
					 and id_vac_apli<>33 --obsoleta exclusion del listado
					 order by nombre";
			 ...
			 while (!$res_efectores->EOF){ 
			    ...
			    $nombre_efector=$res_efectores->fields['nombre'];
			    if ($res_efectores->fields['esta_nomenclador']=='s') $color="background-color: #9CDD95";
			    else $color="background-color: white";	    
			    ?>
				<option value='<?php echo $cuiel?>' style="<?php echo $color?>"><?php echo $nombre_efector?></option>
			    <?
			    $res_efectores->movenext();
			    }?>
			</select>
			...
			<td align="left">			 	
			 ...
			 <option value=-1>Seleccione</option>
			 <?
			 $sql= "SELECT * from trazadoras.dosis_apli
			 			where id_dosis_apli<>7 
			 			order by nombre";
			 $res_efectores=sql($sql) or fin_pagina();
			 while (!$res_efectores->EOF){ 
			    ...
			    $nombre_efector=$res_efectores->fields['nombre'];			    
			    ?>
				<option value='<?php echo $cuiel?>' ><?php echo $nombre_efector?></option>
			    <?
			    $res_efectores->movenext();
			    }?>
			</select>
		  ...
		  <tr>
            ...
            <td align='left'>
              <input type="text" size="40" value="<?php echo $lote?>" name="lote" >
            </td>
			<td align="right">
         	  <b>Nombre del Responsable:</b>
         	</td>         	
            <td align='left'>
              <input type="text" size="40" value="<?php echo $nom_resp?>" name="nom_resp" >
            </td>
		...
		<tr>
            ...
            <td align='left'>
              <input type="text" size="40" value="<?php echo $email?>" name="email" >
            </td>
         	<td align="right">
         	  <b>Observaciones:</b>
         	</td>         	
...
<?// -------------tablas dobes para armar los datos de la vacuna-----------------?>	  
	 ...
	 <tr id="mo">
      ...
      <tr align="center">
       <td>
   <div class="well">
    <input class="btn btn-success" type='submit' name='guardar' value='Guardar' disabled onclick="return control_nuevos()"
     title="Guardar registro de vacunacion">
     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <?php if (!empty($pagina_volver)){ ?>
     <button class="btn btn-primary" onclick="document.location='<?php echo $pagina_volver?>'; return false;">Volver</button>
    <?php } else { ?>
     <button class="btn btn-primary" onclick="document.location='<?php echo $html_root?>/modulos/entrega_leche/listado_beneficiarios_leche.php'; return false;">Volver</button>
    <?php } ?>
    </div>
       </td>
      </tr>
...
if ($entidad_alta=='nu'){//carga de prestacion a paciente NO PLAN NACER
	$query="SELECT *,
			...
			INNER JOIN uad.beneficiarios on trazadoras.vacunas.id_beneficiarios= uad.beneficiarios.id_beneficiarios
			where uad.beneficiarios.id_beneficiarios='$id'  and eliminada < '1' 
...
<tr><td><table width="85%" class="bordes" align="center">
	...
	<td>
	<table id="prueba_vida" border="1" width="85%" style="display:none;border:thin groove"align="center">
	<?
	if ($_POST['guardar']=="Guardar"){?>
		<script>
	 	...
	 	while (!$res_comprobante->EOF) {
			...
			if ($result_vac->recordcount()>0)$color="#F7819F";
	 		...
	 		<tr <?php echo atrib_tr()?>>
		 		<td align="left" bgcolor='<?php echo $color?>' onclick="<?php echo $onclick_elegir?>"><?php echo $res_comprobante->fields['cuie'].' - '.$res_comprobante->fields['nom_efector']?></td>
		 		<td align="center" bgcolor='<?php echo $color?>' onclick="<?php echo $onclick_elegir?>"><?php echo fecha($res_comprobante->fields['f_vacuna'])?></td>		 		
		 		<td align="center" bgcolor='<?php echo $color?>' onclick="<?php echo $onclick_elegir?>"><?php echo fecha($res_comprobante->fields['fecha_vac_prox'])?></td>		 		
		 		<td align="center" bgcolor='<?php echo $color?>' onclick="<?php echo $onclick_elegir?>"><?php echo $res_comprobante->fields['nom_vacum']?></td>		
		 		<td align="center" bgcolor='<?php echo $color?>' onclick="<?php echo $onclick_elegir?>"><?php echo $res_comprobante->fields['dosis']?></td>	
		 		<td align="center" bgcolor='<?php echo $color?>' onclick="<?php echo $onclick_elegir?>"><?php echo $res_comprobante->fields['lote']?></td>	
				<td align="center" bgcolor='<?php echo $color?>' onclick="<?php echo $onclick_elegir?>"><?if ($res_comprobante->fields['nom_resp']!="") echo $res_comprobante->fields['nom_resp']; else echo "&nbsp"?></td>
		 		<td align="center" bgcolor='<?php echo $color?>' onclick="<?php echo $onclick_elegir?>"><?if ($res_comprobante->fields['email']!="") echo $res_comprobante->fields['email']; else echo "&nbsp"?></td>	
		 		<td align="center" onclick="<?//=$onclick_elegir?>"><?if ($res_comprobante->fields['fecha_pcontrol_flag']!="0") echo "NO"; else echo "SI"?></td>			
		 		 <?if (!permisos_check('inicio','perm_borra_vacuna')){		 		 	
					...
					<td align="center"><img src='../../imagenes/salir.gif' style='cursor:hand;' onclick="<?php echo $onclick_provincia?>"></td>
		 		<?}?>		 		 		
		 	</tr>	
