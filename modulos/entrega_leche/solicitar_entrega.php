...
require_once("funciones_leche.php");
...
$remito=round($_POST['remito']); 
$cuie=$_POST['efector'];

if($marcar=="True"){	//
	 $db->StartTrans();
/*$query_sel="SELECT from leche.detalle_leche set
            where id_detalle_leche=$id_detalle_leche";
 $res_q=sql($query_sel, "verificar e1") or fin_pagina();

$detalle=*/

	 $query="UPDATE from leche.detalle_leche set
            borrado=1
			     where id_detalle_leche=$id_detalle_leche";

     sql($query, "verificar E1") or fin_pagina();

        /*cargo los log*/ 
        $usuario=$_ses_user['id'];
        $q="select nextval('leche.log_dleche_id_log_dleche_seq') as id_log_dleche ";
        $id_log=sql($q) or fin_pagina();
        $id_log=$id_log->fields['id_log_dleche '];
        
      $log="INSERT into leche.log_dleche 
           (id_log_dleche, id_detalle_leche, fecha, tipo, descripcion, usuario) 
          values 
          ('$id_log','$id_detalle_leche', 'now()','Eliminado','se procese a eliminacion de registro', '$usuario')";
...
if ($_POST['guardar_form_inicio']=="Guardar Formula Inicio"){	
	...
	elseif ($entidad_alta=='uadben'){
		$id_ben_uad=$id;
    $id_beneficiarios=0;
		$id_smiafiliados=0;
		$sql_benef="SELECT * from leche.formula_inicio where id_ben_uad='$id' and estado='p'";
		$res_sql_benf=sql($sql_benef,"No se pudo ejecutar la consulta en BD") or fin_pagina();
...
if ($id_benuad){
	$sql="SELECT DISTINCT
                          id_beneficiarios as id, 
                          clave_beneficiario, 
                          apellido_benef as a, 
                          nombre_benef as b, 
                          numero_doc as c, 
                          fecha_nacimiento_benef as d, 
                          calle,
                          numero_calle,
                          piso,
                          dpto,
                        ...
                        FROM 
                          uad.beneficiarios
                        WHERE id_beneficiarios=$id_benuad";
...
$e=$res_comprobante->fields['barrio'].', '.$res_comprobante->fields['calle'].', '.$res_comprobante->fields['numero_calle'].', '.$res_comprobante->fields['piso'].', '.$res_comprobante->fields['dpto'].', '.$res_comprobante->fields['manzana'].'('.$res_comprobante->fields[' entre_calle_1'].', '.$res_comprobante->fields['entre_calle_2'].')';
$te=$res_comprobante->fields['telefono'];
...
function control_nuevos()
{
 ...
 if(document.all.motivo.value=="-1"){
  alert('Debe Seleccionar un Tipo de Leche');
  return false;
 }
 ...
 if (isNaN(document.all.remito.value)){
  alert('Debe Ingresar un valor Entero en el Remito');
  return false;
 }
...
<form name='form1' action='solicitar_entrega.php' method='POST' enctype='multipart/form-data'>
...
<input type="hidden" name="id_benuad" value="<?=$id_benuad?>">
<table width="95%" cellspacing=0 border=1 bordercolor=#CDC8E9 align="center" bgcolor='<?=$bgcolor_out?>' >
 <tr class="btn btn-warning">
    <td>
     <font size=+1><b>Registrar Alta de entrega de leche</b></font>    
    </td>
 </tr>
...
<?
  $sql_tmp="SELECT puco.documento,puco.nombre,
            obras_sociales.nombre AS obra_social, 
            obras_sociales.cod_os AS cod_obra_social
				FROM puco.puco
	...
	if( ($query1->RecordCount()==0) || ($estado=='SI') || ($cod_obra_social=='997001') ){?> 
	 ...
	 <table class="bordes" align="center" width="70%">
      ...
      if($res_efectores->RecordCount()==1){
          ...
          <input type="text" id="country_id" onkeyup="autocomplet()" value="<?=$nombre_efectorv?>" autocomplete="off">
            <ul id="country_list_id"></ul>

            <input type="hidden" name="cuie" value="<?=$cuie?>" id="cuie">
        </div>       
    </td>
...
<input type="submit" name="guardar_form_inicio" id="guardar_prestacion" value="Guardar Formula Inicio" title="Guardar Formula Inicia" 
