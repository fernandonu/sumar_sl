...
<?php
	...
	<form name="form1" method="POST" action="calidad_auditorias_detalle.php">
		...
		<input type='hidden' name='id_auditoria_calidad' value='<?php echo $id_auditoria_calidad?>'>
		<input type='hidden' name='afectadosValues' value='<?php echo $afectadosValues?>'>
		<input type='hidden' name='first_time' value='<?php echo $first_time?>'>
		<input type='hidden' name='estado' value='<?php echo $estado?>'>
		<input type='hidden' name='cambio_estado' value='<?php echo $cambio_estado?>'>

		<table border="1" cellspacing="0" bgcolor="<?php echo $bgcolor2?>" width="90%" align="center">
			<th align="center" colspan="4" id="mo"><?php echo $titulo_tabla?></th>
			<tr>
				<td id="mo">
					Título:
				</td>
				<td colspan="3">
					<input type="text" name="t_titulo" id="t_titulo" value="<?php echo $titulo?>" style="width:'100%'">
				</td>
			</tr>
			<tr>
				<td id="mo">Tipo:</td>
				<td>
					<?php echo g_draw_mix_select("sel_tipo", $tipo, array("0"=>array("key"=>"interna", "label"=>"interna"), "1"=>array("key"=>"externa", "label"=>"externa")));?>
				</td>
				<td id="mo">Planta:</td>
				<td>
					<?php echo g_draw_mix_select("sel_planta", $planta, $sels["plantas"]);?>
				</td>
			</tr>
			<tr>
				<td id="mo">Período:</td>
				<td colspan="3" align="left">
					desde <input type="text" name="fecha_desde" id="fecha_desde" value="<?php echo Fecha($fecha_desde)?>" readonly></input>&nbsp;<?php echo link_calendario("fecha_desde")?>
					hasta <input type="text" name="fecha_hasta" id="fecha_hasta" value="<?php echo Fecha($fecha_hasta)?>" readonly></input>&nbsp;<?php echo link_calendario("fecha_hasta")?>
				</td>
			</tr>
		</table>
		<table border="1" cellspacing="0" bgcolor="<?php echo $bgcolor3?>" width="90%" align="center">
		...
		<table border="0" cellspacing="0" bgcolor="<?php echo $bgcolor2?>" width="90%" align="center">
			<tr>
				<td align="center">
		...
		<table border="1" cellspacing="0" bgcolor="<?php echo $bgcolor3?>" width="90%" align="center">
			<tr>
				...
				<?$rta_archivos=sql($sql_archivos) or fin_pagina();?>
					...
					<input type="button" name="bagregar" value="Agregar" onclick="if (typeof(warchivos)=='object' && warchivos.closed || warchivos==false) warchivos=window.open('<?php echo  encode_link($html_root.'/modulos/archivos/archivos_subir.php',array("id_auditoria_calidad"=>$id_auditoria_calidad, "user"=>$_ses_user["name"], "onclickaceptar"=>"window.self.focus();", "proc_file"=>"../calidad/calidad_auditorias_archivos.php")) ?>','','toolbar=0,location=0,directories=0,status=0, menubar=0,scrollbars=1'); else warchivos.focus()" <?php echo $new?>>
				</td>	
			</tr>
<?php
