...
?>
	...
	<table cellspacing=2 cellpadding=2 border=0 bgcolor=<?php echo $bgcolor3 ?> width=95% align=center>
		<tr>
			<td>
		...
		<tr>
			<td align=center>
				<?php list($sql, $total_leg, $link_pagina, $up) = form_busqueda($sql_tmp,$orden,$filtro,$link_tmp,$where_tmp,"buscar"); ?>
				<input type="checkbox" name="sel_periodo" value="avanzado" <?php echo (($_POST["sel_periodo"])?" checked ":"")?> onclick="document.all.tabla_busqueda_avanzada.style.display=(this.checked)?'inline':'none';">&nbsp;Avanzado
				<input type=submit name=buscar value='Buscar'>
			</td>
		</tr>
		<tr>
			<td>
				<table border=0 bgcolor="<?php echo $bgcolor3?>" width=70% align="center" id="tabla_busqueda_avanzada" style="display:none">
					<tr>
						<td>
							<b>Entre el día:&nbsp;</b>
							<input type=text name='fecha_desde' readonly size=11 value='<?php echo $fecha_desde?>'>&nbsp;
							<?php echo link_calendario('fecha_desde');?>
						</td>
						<td>
							<b>y el día:&nbsp;</b>
							<input type=text name='fecha_hasta' readonly size=11 value='<?php echo $fecha_hasta?>'>&nbsp;
	...
	<table align="center" width="90%" cellpadding="1" cellspacing="0" border="1">
		<tr>
			<td colspan="6">
				<table width=100%>
					<tr id=ma>
						<td width=30% align=left><b>Total:</b> <?php echo $total_leg?> auditorías.</td>
		...
		<tr id=mo>
 			...
 			<td nowrap><b><a href='<?php echo encode_link($_SERVER["PHP_SELF"],array("sort"=>"2","up"=>$up))?>'>Título</a></b></td>
 			<td nowrap width="10%"><b><a href='<?php echo encode_link($_SERVER["PHP_SELF"],array("sort"=>"3","up"=>$up))?>'>Tipo</a></b></td>
 			<td width="10%"><b><a href='<?php echo encode_link($_SERVER["PHP_SELF"],array("sort"=>"4","up"=>$up))?>'>Desde</a></b></td>
 			<td width="10%"><b><a href='<?php echo encode_link($_SERVER["PHP_SELF"],array("sort"=>"5","up"=>$up))?>'>Hasta</a></b></td>
 			<td width="5%">&nbsp;</td>
		</tr>
		<?php
		...
		?>
			...
			<td><?php echo Fecha($fila["fecha_desde"])?></td>
			<td><?php echo Fecha($fila["fecha_hasta"])?></td>
			<td>
		<?php
...
?>
		...
		<tr>
			<td colspan="6" align="center" bgcolor="<?php echo $bgcolor3?>">
				<input type="button" name="nueva_auditoria" value="Nueva Auditoría" onclick="document.location.href='<?php echo encode_link("calidad_auditorias_detalle.php",array("modo"=>"nuevo", "pagina"=>"calidad_auditorias_detalle.php"))?>'">
				<input type="button" name="editar_depto" value="Editar departamentos de la empresa" onclick="document.location.href='<?php echo encode_link("editar_piramide.php",array("pagina"=>"calidad_auditorias_detalle.php"))?>'">
			</td>
		</tr>
	</table>
</form>
