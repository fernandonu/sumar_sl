...
<table width="100%">
  <tr>
   <td>
    <table width="100%">
     <tr>
      <td align=left>
       <b>Total Efectores: </b><?php echo $result->RecordCount();?> 
       </td>       
      </tr>      
    </table>  
   </td>
 ...
 <table width="100%" align=center border=1 bordercolor=#585858 cellspacing="0" cellpadding="5"> 
  ...
  while (!$result->EOF) {?>  
    <tr>     
     <td><?php echo $result->fields['cuie']?></td>
     ...
     $r1=sql($sql,"error R1");     
     ?>
     <td><?php echo $r1->fields['r1']?></td>
     <?php $cuie=$result->fields['cuie'];
     $sql = "SELECT count (smiafiliados.id_smiafiliados)as r1 from nacer.smiafiliados 
     ...
     $t1=sql($sql,"error R1");     
     ?>
     <td><?php echo $t1->fields['r1']?></td>
     <?php $cuie=$result->fields['cuie'];
     $sql = "SELECT count (id_nino)as r1 from trazadoras.nino 
     WHERE cuie='$cuie'";
     $t2=sql($sql,"error R1");     
     ?>
     <td><?php echo $t2->fields['r1']?></td>
     <?php $cuie=$result->fields['cuie'];
     $sql = "SELECT count (id_par)as r1 from trazadoras.partos 
     WHERE cuie='$cuie'";
     $t3=sql($sql,"error R1");     
     ?>
     <td><?php echo $t3->fields['r1']?></td>
     <?php $total_tra=$t1->fields['r1']+$t2->fields['r1']+$t3->fields['r1']?>
     <td><?php echo $total_tra?></td>     
     <?$cuie=$result->fields['cuie'];
     $sql = "select sum (monto_egreso) as total from contabilidad.egreso
        where cuie='$cuie'";
     $total_egreso=sql($sql,"error R1");     
     ?>
     <td><?php echo number_format($total_egreso->fields['total'],2,',','.')?></td> 
    </tr>
    <?php $result->MoveNext();
    }?>
 </table>
