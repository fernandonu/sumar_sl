...
function control_nuevos()
{
 ...
 if(document.all.porcentaje.value==0){
  alert('Debe Ingresar un valor positivo mayor de 0 para el porcentaje');
  return false;
 }
 
 if(!Number(document.all.porcentaje.value)){
  alert('Debe Ingresar un valor valido para el porcentaje');
  return false;
 }
...
var importe_parcial = total*(document.all.porcentaje.value/100);

document.all.pago_parcial.readonly = true;
//document.all.pago_parcial.value=total*(document.all.porcentaje.value/100);
document.all.pago_parcial.value = Number(importe_parcial.toFixed(2));
