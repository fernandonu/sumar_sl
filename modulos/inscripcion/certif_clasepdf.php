...
{global $periodo,$id_factura;
	...
	$this->Image('logo_largo_sumar.jpg',115,10,85);
	$this->line(110,5,110,32);
	$this->line(5,32,205,32);
	$this->SetFont('Arial','B',18);
    $this->setxy(15,10);
	$this->Cell(80,10,utf8_decode("Constancia de Inscripción"));
	$this->setxy(15,13);	
...
function activo($activo) {
	$this->SetFont('Arial','B',18);
	$this->setxy(5,32);
	$this->cell(5,9,utf8_decode("Nro. Transacción: $activo"));
}
...
function dibujar_planilla($vencimiento) {
	...
	$this->cell(30,6,utf8_decode('Fecha de Inscripción: '));
	$this->setxy(100,$this->recuperar_base1()+18);
	$this->cell(30,6,'Fecha de Nacimiento: ');
	$this->setxy(7,$this->recuperar_base1()+28);
	$this->SetFont('Arial','B',10);
	$this->cell(200,5,'Por la presente DEJO CONSTANCIA que la persona Antes Detallada esta INSCRIPTO EN EL PLAN SUMAR+.');
	$this->setxy(7,$this->recuperar_base1()+33);
	$this->SetFont('Arial','',10);
	$this->cell(200,5,utf8_decode('Reune los requisitos necesarios para el proceso de validación.'));
	...
	$this->cell(200,5,utf8_decode('VALIDO DENTRO DE LOS 60 DIAS DE LA FECHA DE EMISIÓN. (Vencimiento: '.$vencimiento.')'));
