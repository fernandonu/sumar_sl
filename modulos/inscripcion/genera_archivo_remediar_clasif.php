...
if (!$result12->EOF) {
    ...
    $contenido2.=';';
    $contenido2.='';
    $contenido2.=$fecha_actual;     // 10                                                       HEADER N2
    //$contenido2.=';';
    //$contenido2.=$result12->fields['id_localidad'];
    $contenido2.=';';
    //$contenido2.=$cod_uad;
    $contenido2.=$id_user2;             //10                                                    HEADER N3
    $contenido2.=';';

    if (!$resultP2->EOF) {
        $contenido2.=$cod_prov2; //  2	Dos Primeras Letras? O el Id?  -- el id parece          HEADER N4
        $contenido2.=';';
        $contenido2.=$periodo;  //  6                                                           HEADER N5
...
';
    ...
    while (!$result13->EOF) {
        ...
        $contenido2.='"' . $result13->fields['tipo_documento'] . '"'; //3	Sigla (DNI, CUIL, etc)  DATOS N2
        $contenido2.=';';
        $contenido2.=$result13->fields['numero_doc']; //12  DATOS N3
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['sexo'] . '"'; //1	M / F                                   DATOS N4
        $contenido2.=';';
        $contenido2.=$result13->fields['fecha_nacimiento_benef']; //10	AAAA-MM-DD (A o, Mes, D a)      DATOS N5
        $contenido2.=';';
        $contenido2.=substr($result13->fields['fecha_control'], 0, 10); // fecha clasificacion          DATOS N6
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['calle'] . '"'; //40                                                           DATOS N7
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['numero_calle'] . '"'; //5                                 DATOS N8
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['manzana'] . '"'; //5                                      DATOS N9
        $contenido2.=';';
        $contenido2.=$result13->fields['piso']; //5                                                     DATOS N10
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['dpto'] . '"'; //5                                         DATOS N11
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['entre_calle_1'] . '"'; //40                               DATOS N12
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['entre_calle_2'] . '"'; //40                               DATOS N13
        $contenido2.=';';

        $contenido2.='"' . $result13->fields['barrio'] . '"'; //40               DATOS N14
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['municipio'] . '"'; //40            DATOS N15
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['departamento'] . '"'; //40         DATOS N16
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['localidad'] . '"'; //40            DATOS N17
        $contenido2.=';';
        $contenido2.=$result13->fields['cod_pos']; // 8 DomCodigoPostal                                 DATOS N18
        $contenido2.=';';
        $contenido2.=$cod_prov2; // 2 $result12->fields['provincia_nac'];                               DATOS N19
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['telefono'] . '"'; //20                                    DATOS N20
        $contenido2.=';';
        $contenido2.=substr($result13->fields['fecha_carga'], 0, 10); // 10 FechaCarga                  DATOS N21
        $contenido2.=';';
        $contenido2.='"' . $result13->fields['codremediar'] . '"'; // 15 Efector                        DATOS N22
        $contenido2.=';';
