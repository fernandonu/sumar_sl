...
function expediente_anulacion($id_factura) {
    
  $sql_trans="delete from expediente.transaccion where id_factura=$id_factura";
  $res_trans=sql($sql_trans,"No se pudo dar de baja al expediente en transacciones") or fin_pagina();
  
  $sql_expediente="delete from expediente.expediente where id_factura=$id_factura";
  $res_expediente=sql($sql_expediente,"No se pudo dar de baja al registro") or fin_pagina();
  
  $sql_fact="update facturacion.factura set estado='X',traba='',estado_exp=0 where id_factura=$id_factura";
  $res_sql=sql($sql_fact,"No se pudo actualizar los estados de la factura") or fin_pagina();
  
 }



$sql_tmp="SELECT * FROM expediente.expediente 
left join nacer.efe_conv using (id_efe_conv) 
left join expediente.transaccion using (id_expediente) where id_expediente='$id_expediente'";
...
if ($_POST['guardar']=="Guardar"){   
  ...
  if ($estado=='R'){
    $db->StartTrans();
    expediente_anulacion($id_factura);
    $db->CompleteTrans();
    
    echo"<script language='javascript'>window.location='../calidad/detalle_no_conformes.php'</script>;";
  }
  else {
  ...
  $db->StartTrans();         
    ...
    switch ($estado) {
      ...
      case "D": $control=3;$id_area=1;break; 
      };
...
if (document.all.estado.value=="R"){
    return (confirm('Esta seguro que desea ANULAR la factura del sistema de Expediente'));
    }
