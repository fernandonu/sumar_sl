...
$filtro = array(
        "to_char(id_factura,'999999')" => "Nro. Factura",
		    "efe_conv.cuie" => "CUIE",
        "expediente.nro_exp" => "nro_exp",
		    "nombre" => "Nomb.Efector"               
       );

$sql_tmp="Select expediente.*,efe_conv.*,factura.id_factura,factura.alta_comp
...
if ($selec_fecha=='expediente.fecha_ing'){ $criterio_filtro="Fecha Ingreso al Sistema de Expediente";
	   $where_tmp="fecha_ing between '$fecha_desde' and '$fecha_hasta'"; 	
	}
if ($selec_fecha=='expediente.plazo_para_pago'){ $criterio_filtro="Fecha de Pago de Acuerdo a Expediente->plazo_para_pago";
		$where_tmp="plazo_para_pago between '$fecha_desde' and '$fecha_hasta'";
		}
}
...
echo $html_header;
?>
<form name="form1" action="historial2015.php" method=POST>
...
<tr>
    <td align=right id="mo"><a id="mo" href='<?=encode_link("historial.php",array("sort"=>"1","up"=>$up))?>'>Numero de Expediente</a></td>
     ...
     <td align=right id="mo"><a id="mo" href='<?=encode_link("historial.php",array("sort"=>"5","up"=>$up))?>'>Tipo de Fact.</a></td> 
...
<?
    ...
    <?$tipo_factura=($result->fields['alta_comp']=='SI') ? 'PPAC' : 'Normal';?>
    <td <?=$tr?> onclick="<?=$onclick_elegir?>" align="center"><b><?=$tipo_factura?></b></td>
