...
function cargar_medicos($id_especialidad, $id_efector) {
  ...
  $query = "SELECT 
            ...
            FROM
              nacer.agendas a
              LEFT OUTER JOIN nacer.especialidades_medicos em ON (em.id = a.id_especialidad_medico)
              LEFT OUTER JOIN nacer.especialidades e ON (e.id_especialidad = em.id_especialidad)
              LEFT OUTER JOIN nacer.medicos_efectores me ON (a.id_medico_efector = me.id)
              LEFT OUTER JOIN nacer.medicos m ON (me.id_medico = m.id_medico)
            WHERE
              e.id_especialidad = $id_especialidad AND 
  ...
  if ($res_medicos->recordCount() > 0) {
    ...
    foreach ($medicos as $medico => $datos) {
      ...
      if (!empty($datos["agendas"])) {
        foreach ($datos["agendas"] as $agenda) {
          ...
          while (strtotime('+'.$duracion.' minutes', $hora_inicio) <= $hora_fin) {
            echo "<div class='external-event' style='background-color: ", $medico_colores[$color_index], "' data-id='{$id}-", $count_id++, "' data-start='", date("H:i:s", $hora_inicio), "' data-end='", date("H:i:s", strtotime('+'.$duracion.' minutes', $hora_inicio)), "' data-duration='", date("H:i:s", mktime(0, $duracion, 0)), "'><span class='datos-agenda'>", date("H:i", $hora_inicio), "</span></div>";
      ...
      echo "</div>";
      $color_index++;
          if ($color_index == count($medico_colores)) {
            $color_index = 0;
      }
    }
