...
$id_usuario = $_ses_user['id']; //$_ses_user['id']; el usuario logueado actualmente
$usuario1=$_ses_user['id'];
//valido que el usuario sea medico con agenda asignada
$q_ry="SELECT DISTINCT 
          nacer.agendas.id_medico_efector,
          nacer.agendas.id as id_agenda,
          nacer.efe_conv.id_efe_conv,
          nacer.agendas.id_especialidad_medico
        FROM
          ...
          INNER JOIN nacer.efe_conv ON (nacer.medicos_efectores.id_efector = nacer.efe_conv.id_efe_conv)
          INNER JOIN sistema.usu_efec ON (nacer.medicos.medico_user = sistema.usu_efec.id_usuario)
          INNER JOIN nacer.agendas ON (nacer.agendas.id_medico_efector = nacer.medicos_efectores.id)
        Where nacer.medicos.medico_user=$usuario1";
  $resqy=sql($q_ry,"al verificar si es medico");
if(!$resqy->EOF)$user_medico=1;else $user_medico=2;
...
if ($id_efector) {
  if($user_medico==2){ echo "entro por if 1 ".$user_medico;
      	$query = "SELECT 
      ...
      $query = "SELECT DISTINCT 
              ...
              FROM
                ...
                INNER JOIN sistema.usu_efec ON (nacer.medicos.medico_user = sistema.usu_efec.id_usuario)
                INNER JOIN nacer.agendas ON (nacer.agendas.id_medico_efector = nacer.medicos_efectores.id)
                INNER JOIN nacer.especialidades ON (nacer.especialidades_medicos.id_especialidad = nacer.especialidades.id_especialidad)
                Where nacer.medicos.medico_user=$id_usuario
                ORDER BY
                nacer.especialidades.nom_titulo";
}
...
//             FROM
//               nacer.medicos m
//               LEFT OUTER JOIN nacer.especialidades_medicos em ON (em.id_medico = m.id_medico)
//               LEFT OUTER JOIN nacer.medicos_efectores me ON (m.id_medico = me.id_medico)
//             WHERE
//               em.id_especialidad = $id_especialidad AND 
//               me.id_efector = $id_efector
//             ORDER BY
//               apellido_medico,
//               nombre_medico";
...
<?php $paciente_display = ($id_paciente) ? 'block' : 'none'; ?>
	<div class="col-sm-6">
    <div class="paciente-datos" style="display: <?php echo $paciente_display; ?>;">
...
<br/>
<div class="row">
	<div class="col-xs-3" id="columna-turnos">
	</div>
	<div class="col-xs-9">
		<div id='calendar'></div>
	</div>
...
<script type="text/javascript">
  ...
  var events_url = '<?php echo encode_link("agenda_datos.php", array("accion" => "agenda_events")); ?>';
	...
	function cargar_medicos(id_especialidad, id_efector) {
		$.ajax({
			url: '<?php echo encode_link("agenda_datos.php", array("accion" => "cargar_medicos")); ?>',
			type: 'POST',
			data: { 'id_especialidad': id_especialidad, 'id_efector': id_efector },
			success: function(datos) {
  ...
  function check_duplicado_medic_pac(event){
    var eventos = $('#calendar').fullCalendar('clientEvents');
    for(i in eventos){
      if(eventos[i].medico == event.medico && eventos[i].rendering != 'background') {
        if(!(eventos[i].start.unix() >= event.end.unix() || eventos[i].end.unix() <= event.start.unix())) {
          return true;
        }
      }
  ...
  function get_event_color(id_agenda) {
	...
	$(document).ready(function() {
		<?php 
		if ($id_especialidad) {
			echo "cargar_medicos(${id_especialidad}, ${id_efector});";
		}
		?>
		...
		$('#btn-estado').on('click', function() {
			$.ajax({
				...
				type: 'POST',
        data: 'id_turno='+$('#detalle_id_turno').val()+'&motivo='+$('#motivo').val()+'&sobreturno='+$('#sobreturno:checked').val(),
				success: function(resultado) {
					if (resultado == 1) {
						// BootstrapDialog.alert("El turno fu&eacute; cancelado correctamente");
						$('#calendar').fullCalendar( 'refetchEvents' );
    ...
    $('#btn-diagnostico').on('click', function() {
      $.ajax({
        url: '<?php echo encode_link("c_diagnistico.php", array("accion" => "carga_diagnistico")); ?>',
        type: 'POST',
        data: 'id_turno='+$('#detalle_id_turno').val(),
      ...
      var id_especialidad = $(this).val();
			var id_efector = $('#id_efector').val();
			cargar_medicos(id_especialidad, id_efector);
		});
    	...
    	// maxTime: '20:00:00',
  		...
  		eventRender: function( event, element, view ) {
  			if(view.name == 'agendaDay') {
	  			...
	  			if ((event.estado == 'presente')||(event.estado == 'completo')) {
        ...
        msg += '<div class="row"><div class="col-md-3 text-right"><b>DNI:</b></div><div class="col-md-9">'+event.dni+'</div></div>';
				msg += '<div class="row"><div class="col-md-3 text-right"><b>Domicilio:</b></div><div class="col-md-9">'+event.domicilio+'</div></div>';
        msg += '<div class="row"><div class="col-md-3 text-right"><b>Obra Social:</b></div><div class="col-md-9">'+event.obra_social+'</div></div>';
        msg += '<div class="row"><div class="col-md-3 text-right"><b>Motivo:</b></div><div class="col-md-9"><input type="text" id="motivo" name="motivo" value=""></div></div>';
				msg += '<div class="row"><div class="col-md-3 text-right"><b>Sobreturno:</b></div><div class="col-md-9"><input type="checkbox" id="sobreturno" value="sobreturno"></div></div>';
      ...
      eventDrop: function(event, delta, revertFunc, jsEvent, ui, view) {
        ...
        var hoy = new Date();
        if ( (date) < (hoy.setDate(hoy.getDate() - 5)) ) {
          $.notify("No se puede asignar un turno con fecha anterior", "error");
          return false;
        }
				...
				if (check_duplicado(copiedEventObject)) {
					$.notify("Turno duplicado por Paciente", "error");
					return false;
				}
        ...
        if (check_duplicado_medic_pac(copiedEventObject)) {
          if (!confirm("¿Esta Seguro que desea Superponer Pacientes?")){
            $.notify("Turno duplicado, con el mismo Medico", "error");
            return false;
          }
        }
