...
if ($_POST["agenda_accion"] == "modificar") {
  ...
  $id_medico_efector = $_POST["agenda_efector"];
  if (empty($id_medico_efector) || !is_numeric($id_medico_efector)) {
    $error .= "Falta seleccionar el efector<br/>";
  }
	...
	if (empty($error)) { // No hubo errores
		$query="UPDATE nacer.agendas SET
            id_especialidad_medico=".$db->Quote($id_especialidad).",
						id_medico_efector=".$db->Quote($id_medico_efector).",
...
if ($_POST["agenda_accion"] == "agregar") {
	...
	$id_medico_efector = intval($_POST["agenda_efector"]);
  if (empty($id_medico_efector) || $id_medico_efector <= 0) {
    $error .= "Falta seleccionar el efector<br/>";
  }
  ...
  $id_especialidad = intval($_POST["agenda_especialidad"]);
	...
	if (empty($error)) { // No hubo errores
		$query="INSERT INTO nacer.agendas (id_especialidad_medico, dom, lun, mar, mie, jue, vie, sab, hora_inicio, hora_fin, fecha_inicio, fecha_fin, duracion, id_medico_efector) 
		...
		$id_especialidad = $hora_inicio = $hora_fin = $fecha_inicio = $fecha_fin = $duracion = $id_medico_efector = "";
		$dias_values = array();
...
if ($_POST['guardar_editar']=='Guardar'){
  ...
  $codigo_postal  = trim($_POST['codigo_postal']);
	$medico_user 	= trim($_POST['medico_user']);
	...
	if (empty($error)) { // No hubo errores
		...
		$query="UPDATE nacer.medicos SET 
          ...
          cod_postal=".$db->Quote($codigo_postal).",
					medico_user=".$db->Quote($medico_user)."
	...
	if (empty($error)) { // No hubo errores
		...
		if ($res_verificar->recordCount()==0) {
			$query="INSERT INTO nacer.medicos
			   			(nombre, apellido, fecha_nac, telefono, email, sexo, dni, domicilio, cod_postal, medico_user)
			   		VALUES
			   			(
			   			".$db->Quote($nombre).", 
			   			".$db->Quote($apellido).", 
...
if ($id_medico) {
	...
	$query_agenda = "SELECT 
            ...
            LEFT OUTER JOIN nacer.medicos_efectores ON (nacer.agendas.id_medico_efector = nacer.medicos_efectores.id)
            LEFT OUTER JOIN nacer.efe_conv ON (nacer.medicos_efectores.id_efector = nacer.efe_conv.id_efe_conv)
					WHERE
...
<script type="text/javascript">
	...
	function validar_agenda() {
		var efector = $('#agenda-efector').val();
    var especialidad = $('#agenda-especialidad').val();
		...
		if (efector == '') {
      alert('Debe seleccionar el Efector');
      return false;
    }
    if (especialidad == '') {
	...
	$(document).ready(function() {
		...
		$('#add-edit-agenda').on('show.bs.modal', function(e) {
			if ($(e.relatedTarget).hasClass('btn-agregar-agenda')) {
				...
				if (row_id) {
					$('#agenda-form #agenda_efector').val('Modificar');
          $('#agenda-form #agenda-efector option').filter(function () { 
            return $(this).html() == $('#'+row_id+' .agenda-efector').html();
          }).prop('selected', true);
		...
		<td>
			<table width="90%" align="center" class="bordes">
			    ...
			    <tr>
                  ...
                  <tr>
                    <td align="right"><b>Login de Usuario:</b></td>
                    <td align="left">
                      <select name="medico_user" Style="width=450px" id="medico_user">
                        <option value=-1>Seleccione</option>
                         <?
                         $sql= "SELECT id_usuario, login, nombre, apellido FROM sistema.usuarios ORDER BY nombre, apellido";
                         $res_user=sql($sql) or fin_pagina();
                         while (!$res_user->EOF){ 
                            $id_usuario=$res_user->fields['id_usuario'];
                            $nombre=$res_user->fields['login']." - ".$res_user->fields['nombre'].", ".$res_user->fields['apellido'];?>
                            <option value="<?=$id_usuario;?>" <?if ($id_usuario==$medico_user)echo "selected"?> ><?=$nombre?>                                
                            </option>
                            <?$res_user->movenext();
                          }?>
                        </select>
							...
							<div id="agendas" class="panel-collapse collapse">
								<div class="panel-body text-center">
									<table class="table table-condensed table-hover">
										<thead>
											<tr>
												<th width="30%" class="small">Horario</th>
                        <th width="30%" class="small">Efector</th>
												<th width="30%" class="small">Especialidad</th>
												<th width="10%" class="small">Acciones</th>
											</tr>
										</thead>
										<tbody>
											...
											if ($res_agenda->recordCount() > 0) {
							      				while (!$res_agenda->EOF) {
													...
													echo '</td>';
                          echo '<td><span class="agenda-efector">', $res_agenda->fields['nombre_efector'], '</span></td>';
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="row">
                <div class="col-md-12">
                  <label for="agenda-efector">Efector</label>
                  <select class="form-control" name="agenda_efector" id="agenda-efector">
              <option></option>
              <?php
              if ($res_efe->recordCount() > 0) {
                $res_efe->MoveFirst();
                while (!$res_efe->EOF) {
                  $selected = "";
                  if (!empty($id_medico_efector) && $id_medico_efector == $res_efe->fields['id']) {
                    $selected = " selected";
                  }
                  echo '<option value="', $res_efe->fields['id'], '"', $selected, '>', $res_efe->fields['efector_nombre'], '</option>';
                  $res_efe->MoveNext();
                }
              }
              ?>
