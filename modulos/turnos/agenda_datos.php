...
if (isset($parametros["accion"])) {
  switch ($parametros["accion"]) {
    ...
    case 'cargar_medicos':
      ...
      $id_efector = intval($_POST["id_efector"]);
      if ($id_especialidad > 0 && $id_efector > 0) {
        $_ses_agenda_turnos["id_especialidad"] = $id_especialidad;
        $_ses_agenda_turnos["id_efector"] = $id_efector;
    ...
    case 'agenda_events':
      ...
      if ($id_efector > 0) {
        $query = "SELECT 
                  ...
                  WHERE
                    nacer.agendas_eventos.estado IN ('activo', 'presente', 'completo') AND 
        ...
        while (!$res_eventos->EOF) {
          $res_json[] = array(
            ...
            "domicilio"     => $res_eventos->fields["calle"].', '.$res_eventos->fields["numero_calle"],
            "dni"           => $res_eventos->fields["numero_doc"],
    ...
    case 'cancelar_turno':
      ...
      $query="UPDATE nacer.agendas_eventos 
      ...
      if ($dni > 0) {
        ...
        while (!$res_paciente->EOF) {
          $res_json[] = array(
            ...
            "apellido"          => $res_paciente->fields["apellido_benef"],
            "nombre"            => $res_paciente->fields["nombre_benef"],
