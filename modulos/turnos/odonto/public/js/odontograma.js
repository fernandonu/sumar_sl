jQuery(function(){
    ...
    //pinta lo que viene de la base llama a la base por cada diente

    function buscadiente(id1, id2, id3, id4, id5, my_callback){
          var resp;
          var parametros = {
                  "id1" : id1,
                  "id2" : id2,
          ...
          $.ajax({
                  data:  parametros,
                  url:   '../guarda_diente.php',
                  type:  'post',
                  beforeSend: function () {
                         $("#resultado1").html(" Cargando, espere por favor... ");
                  },
                  success:  function (response) {
                    if (id2==18)$("#resultado1").html("");
                    resp=response;
                    my_callback(resp);
    ...
    buscadiente (id_paciente.value,diente.id.toString(),"","","buscadiente",function(resp){
       if(resp!='0')
        {
          for (var i = resp.length - 1; i >= 0; i--) {
            var res = resp.substring(i, i+1);
            caras[res].attr('fill', 'red');
          };         
        }
        else{
        ...
        //guarda en la base
        function realizaProceso(id1, id2, id3, id4, id5){
          var parametros = {
                  "id1" : id1,
                  "id2" : id2,
                  "id3" : id3,
        ...
        realizaProceso(id_paciente.value,diente.id, cara, tratamiento.nombre,"guardadiente");
