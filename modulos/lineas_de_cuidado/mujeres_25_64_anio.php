...
limpiar_tablas('mujeres_25_64_anio','hist_mujeres_25_64_anio');
...
function busqueda_pres_madre($descripcion,$st_codigo,$st_fecha,$st_cuie,$tipo_linea,$fecha_desde,$fecha_hasta,$cuie){
 ...
 if ($cuie=="todos") {
     $sql_1="SELECT comprobante.id_smiafiliados,prestacion.fecha_prestacion,comprobante.cuie,
      ...
      --tomo la edad que tiene el benefic. al momento de las prestacion.
      --del grupo de 25 a 64 año 
      and extract (year from (age(fecha_prestacion,fecha_nacimiento)))>=25 
...
if ($res_sql->recordcount() > 0){
  ...
  while (!$res_sql->EOF) {
        ...
        if (!se_encuentra($id_smiafiliados,$tipo_linea,'mujeres_25_64_anio')) {
        $sql_2="SELECT nextval ('cobertura_efectiva.mujeres_25_64_anio_id_seq') as id";
        $res_sql_2=sql($sql_2,"error al traer el id de la tabla de ninios menores de 1 anio");
        $id=$res_sql_2->fields['id'];
        
        $sql_insert="INSERT into cobertura_efectiva.mujeres_25_64_anio
        ...
        $sql_2="SELECT nextval ('cobertura_efectiva.hist_mujeres_25_64_anio_id_seq') as id";
        $res_sql_2=sql($sql_2,"error al traer el id de la tabla historica de ninios menores de 1 anio");
        $id_hist=$res_sql_2->fields['id'];
        
        $sql_insert="INSERT into cobertura_efectiva.hist_mujeres_25_64_anio
        ...
        $sql_update="UPDATE cobertura_efectiva.mujeres_25_64_anio
        ...
        $sql_consulta="SELECT id from cobertura_efectiva.mujeres_25_64_anio 
                        WHERE id_smiafiliados=$id_smiafiliados and tipo_linea='$tipo_linea'";
...
//CATEGORIA I 
if ($linea_cuidado=='Cancer de Cuello') {
//BUSQUEDA DE PRESTACIONES MADRES - LINEA DE CUIDADO: CANCER DE CUELLO
busqueda_pres_madre('Diagnóstica y seguimiento de CA cervicouterino (inicial)Grupo Mujeres 20-64 años','ctc001a97','fecha_p1','cuie_p1','Cancer de Cuello',$fecha_desde,$fecha_hasta,$cuie);
busqueda_pres_madre('Diagnóstica y seguimiento de CA cervicouterino (inicial)Grupo Mujeres 20-64 años','ctc008a97','fecha_p2','cuie_p2','Cancer de Cuello',$fecha_desde,$fecha_hasta,$cuie);

//BUSQUEDA DE PRACTICAS DERIVADAS
busqueda_pres_compl_2('Toma de muestra citológica ( 25 a 64  años)  (Tamizaje CA cervicouterino)Grupo Mujeres 20-64 años','prp018a98','fecha_p3','cuie_p3','Cancer de Cuello',$fecha_desde,$fecha_hasta,'hist_mujeres_25_64_anio','fecha_p1','fecha_p2');

set_valores('mujeres_25_64_anio','Cancer de Cuello',1);
};
