...
limpiar_tablas('hombres_20_64_anio','hist_hombres_20_64_anio');
...
if ($res_sql->recordcount() > 0){
  ...
  while (!$res_sql->EOF) {
        ...
        if (!se_encuentra($id_smiafiliados,$tipo_linea,'hombres_20_64_anio')) {
        $sql_2="SELECT nextval ('cobertura_efectiva.hombres_20_64_anio_id_seq') as id";
        $res_sql_2=sql($sql_2,"error al traer el id de la tabla de ninios menores de 1 anio");
        $id=$res_sql_2->fields['id'];
        
        $sql_insert="INSERT into cobertura_efectiva.hombres_20_64_anio
        ...
        $sql_2="SELECT nextval ('cobertura_efectiva.hist_hombres_20_64_anio_id_seq') as id";
        $res_sql_2=sql($sql_2,"error al traer el id de la tabla historica de ninios menores de 1 anio");
        $id_hist=$res_sql_2->fields['id'];
        
        $sql_insert="INSERT into cobertura_efectiva.hist_hombres_20_64_anio
        ...
        $sql_update="UPDATE cobertura_efectiva.hombres_20_64_anio
        ...
        $sql_consulta="SELECT id from cobertura_efectiva.hombres_20_64_anio 
                        WHERE id_smiafiliados=$id_smiafiliados and tipo_linea='$tipo_linea'";
...
//CATEGORIA I 
if ($linea_cuidado=='Hombre Sano') {
//BUSQUEDA DE PRESTACIONES MADRES - LINEA DE CUIDADO: HOMBRE SANO
busqueda_pres_madre('Control periódico de salud  del hombre (inicial) ( grupo hombres 20 a 64 años )','ctc001a97','fecha_p1','cuie_p1','Hombre Sano',$fecha_desde,$fecha_hasta,$cuie);

//BUSQUEDA DE PRACTICAS DERIVADAS
busqueda_pres_compl('Control odontológico ( grupo hombres 20 a 64 años )','ctc010a97','fecha_p2','cuie_p2','Hombre Sano',$fecha_desde,$fecha_hasta,'hombres_20_64_anio','hist_hombres_20_64_anio','fecha_p1');
busqueda_pres_compl('Consulta oftalmológica ( grupo hombres 20 a 64 años )','ctc011a97','fecha_p3','cuie_p3','Hombre Sano',$fecha_desde,$fecha_hasta,'hombres_20_64_anio','hist_hombres_20_64_anio','fecha_p1');

set_valores('hombres_20_64_anio','Control de Salud',1);
};
