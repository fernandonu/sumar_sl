...
limpiar_tablas('hombres_40_64_anio','hist_hombres_40_64_anio');
...
function busqueda_pres_madre($descripcion,$st_codigo,$st_fecha,$st_cuie,$tipo_linea,$fecha_desde,$fecha_hasta,$cuie){
 ...
 if ($cuie=="todos") {
     $sql_1="SELECT comprobante.id_smiafiliados,prestacion.fecha_prestacion,comprobante.cuie,
      ...
      --del grupo de 40 a 64 año 
      and extract (year from (age(fecha_prestacion,fecha_nacimiento)))>=40 
      and extract (year from (age(fecha_prestacion,fecha_nacimiento)))<=64
      and  smiafiliados.afisexo='M                   '
...
if ($res_sql->recordcount() > 0){
  ...
  while (!$res_sql->EOF) {
        ...
        if (!se_encuentra($id_smiafiliados,$tipo_linea,'hombres_40_64_anio')) {
        $sql_2="SELECT nextval ('cobertura_efectiva.hombres_40_64_anio_id_seq') as id";
        $res_sql_2=sql($sql_2,"error al traer el id de la tabla de ninios menores de 1 anio");
        $id=$res_sql_2->fields['id'];
        
        $sql_insert="INSERT into cobertura_efectiva.hombres_40_64_anio
        ...
        $sql_2="SELECT nextval ('cobertura_efectiva.hist_hombres_40_64_anio_id_seq') as id";
        $res_sql_2=sql($sql_2,"error al traer el id de la tabla historica de ninios menores de 1 anio");
        $id_hist=$res_sql_2->fields['id'];
        
        $sql_insert="INSERT into cobertura_efectiva.hist_hombres_40_64_anio
        ...
        $sql_update="UPDATE cobertura_efectiva.hombres_40_64_anio
        ...
        $sql_consulta="SELECT id from cobertura_efectiva.hombres_40_64_anio 
                        WHERE id_smiafiliados=$id_smiafiliados and tipo_linea='$tipo_linea'";
...
//CATEGORIA I 
if ($linea_cuidado=='Deteccion de enfermedades cronicas no trasmisibles') {
//BUSQUEDA DE PRESTACIONES MADRES - LINEA DE CUIDADO: DETENCION DE ENFERMEDADES CRONICAS NO TRASMISIBLES
busqueda_pres_madre('Control periódico de salud  del hombre (inicial) ( grupo hombres 20 a 64 años )','ctc001a97','fecha_p1','cuie_p1','Deteccion de enfermedades cronicas no trasmisibles',$fecha_desde,$fecha_hasta,$cuie);

//BUSQUEDA DE PRACTICAS DERIVADAS
busqueda_pres_compl('Consulta para la evaluación del nivel de riesgo cardiovascular en población de riesgo ( grupo hombres 20 a 64 años )','ctc048vdm','fecha_p2','cuie_p2','Deteccion de enfermedades cronicas no trasmisibles',$fecha_desde,$fecha_hasta,'hombres_40_64_anio','hist_hombres_40_64_anio','fecha_p1');

busqueda_pres_compl('Notificación del nivel de riesgo cardiovascular < 10% ( grupo hombres 20 a 64 años )','ntn007k22','fecha_p3','cuie_p3','Deteccion de enfermedades cronicas no trasmisibles',$fecha_desde,$fecha_hasta,'hombres_40_64_anio','hist_hombres_40_64_anio','fecha_p1');

busqueda_pres_compl('Notificación del nivel de riesgo cardiovascular   10%-< 20% ( grupo hombres 20 a 64 años )','ntn008k22','fecha_p4','cuie_p4','Deteccion de enfermedades cronicas no trasmisibles',$fecha_desde,$fecha_hasta,'hombres_40_64_anio','hist_hombres_40_64_anio','fecha_p1');

busqueda_pres_compl('Notificación del nivel de riesgo cardiovascular 20%-< 30% ( grupo hombres 20 a 64 años )','ntn009k22','fecha_p5','cuie_p5','Deteccion de enfermedades cronicas no trasmisibles',$fecha_desde,$fecha_hasta,'hombres_40_64_anio','hist_hombres_40_64_anio','fecha_p1');

busqueda_pres_compl('Notificación del nivel de riesgo cardiovascular  ? 30% ( grupo hombres 20 a 64 años )','ntn010k22','fecha_p6','cuie_p6','Deteccion de enfermedades cronicas no trasmisibles',$fecha_desde,$fecha_hasta,'hombres_40_64_anio','hist_hombres_40_64_anio','fecha_p1');


set_valores('hombres_40_64_anio','Deteccion de enfermedades cronicas no trasmisibles',1);
};
