...
limpiar_tablas('hombres_50_64_anio','hist_hombres_50_64_anio');
...
function busqueda_pres_madre($descripcion,$st_codigo,$st_fecha,$st_cuie,$tipo_linea,$fecha_desde,$fecha_hasta,$cuie){
 ...
 if($cuie=="todos"){
     ...
     --tomo la edad que tiene el benefic. al momento de ls pestacion.
      ...
      and extract (year from (age(fecha_prestacion,fecha_nacimiento)))>=50 
...
if ($res_sql->recordcount() > 0){
  ...
  while (!$res_sql->EOF) {
        ...
        if (!se_encuentra($id_smiafiliados,$tipo_linea,'hombres_50_64_anio')) {
        $sql_2="SELECT nextval ('cobertura_efectiva.hombres_50_64_anio_id_seq') as id";
        $res_sql_2=sql($sql_2,"error al traer el id de la tabla de ninios menores de 1 anio");
        $id=$res_sql_2->fields['id'];
        
        $sql_insert="INSERT into cobertura_efectiva.hombres_50_64_anio
        ...
        $sql_2="SELECT nextval ('cobertura_efectiva.hist_hombres_50_64_anio_id_seq') as id";
        $res_sql_2=sql($sql_2,"error al traer el id de la tabla historica de ninios menores de 1 anio");
        $id_hist=$res_sql_2->fields['id'];
        
        $sql_insert="INSERT into cobertura_efectiva.hist_hombres_50_64_anio
        ...
        $sql_update="UPDATE cobertura_efectiva.hombres_50_64_anio
        ...
        $sql_consulta="SELECT id from cobertura_efectiva.hombres_50_64_anio 
                        WHERE id_smiafiliados=$id_smiafiliados and tipo_linea='$tipo_linea'";
...
//CATEGORIA I 
if ($linea_cuidado=='Deteccion de Riesgo y Diagnostico oportuno de Cancer Colorectal') {
//BUSQUEDA DE PRESTACIONES MADRES - LINEA DE CUIDADO: DETECCION DE RIESGO Y DIAGNOSTICO OPORTUNO DE CANCER COLORECTAL
busqueda_pres_madre('Control periódico de salud  del hombre (inicial) ( grupo hombres 20 a 64 años )','ctc001a97','fecha_p1','cuie_p1','Deteccion de Riesgo y Diagnostico oportuno de Cancer Colorectal',$fecha_desde,$fecha_hasta,$cuie);

//BUSQUEDA DE PRACTICAS DERIVADAS
busqueda_pres_compl('Control periódico de salud  del hombre (inicial) ( grupo hombres 20 a 64 años )','ctc001vdm','fecha_p2','cuie_p2','Deteccion de Riesgo y Diagnostico oportuno de Cancer Colorectal',$fecha_desde,$fecha_hasta,'hombres_50_64_anio','hist_hombres_50_64_anio','fecha_p1');


set_valores('hombres_50_64_anio','Deteccion de Riesgo y Diagnostico oportuno de Cancer Colorectal',1);
};
