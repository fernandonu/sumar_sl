...
if ($id_efe_conv) {
	...
	else $cuie=substr($_ses_user['login'],0,6);
	//$cuie=$_ses_user['login'];
	$sql_cuie="SELECT 
			efe_conv.*,dpto.nombre as dpto_nombre
			FROM
 ...
 <tr><td><table width=90% align="center" class="bordes">
  <tr align="center">
    <td>
     ...
     <input type="button" class="btn btn-primary" name="cumplimiento" value="Evaluacion de Cumplimientos" onclick="<?=$onclick_elegir_det?>" title="Evaluacion de Cumplimientos">     
   </td>
   ...
   <td>
     ...
     <input type="button" class="btn btn-primary" name="gestion" value="Detalle de Gestion" onclick="<?=$onclick_elegir_det_1?>" title="Detalle de Gestion">     
   </td>
   <?
   if (!es_cuie($user)){ ?>
		<td>
