...
$fecha_factura=fecha($res_sql->fields['fecha_factura']);
...
$pdf->Text(110,16,"Fecha Factura:");
$pdf->SetFont('Arial','',10);
$pdf->Text(140,16,"$fecha_factura");
...
$pdf->SetFont('Arial','B',8);
$pdf->Text(166,110,"P.Unitario"); //cantidad
$pdf->SetFont('Arial','B',8);
$pdf->Text(181,110,"Cantidad");//Importe
$pdf->Text(199,110,"Total");//diferencia
...
while (!$res_comp->EOF){
	...
	switch ($id_prestacion){
		case 21:{$pdf->SetFont('Arial','',6);
				...
				$pdf->Text(72,$y_com,"$id_modulo"); //$id_modulo....($ $valor_con_formato)
				break;}
		...
		default: {$pdf->Text(60,$y_com,"$codigo");
    				...
    				$modulo=str_replace('_',' ',$res_comp->fields['id_modulo']);
    				...
    				if (strlen($practica)<=95 ){
						$pdf->Text(72,$y_com,"$practica");} 
					else {	$practica_1=$patologia.".//.".$cirugia;
							$pdf->Text(72,$y_com,"$practica_1");;
							$y_com=$y_com+5;	 
							$pdf->Text(72,$y_com,"$modulo");}
							break;}
			}
		  	
    $pdf->Text(167,$y_com,"$ $valor_con_formato");//anterior $cantidad
	$pdf->Text(187,$y_com,"$cantidad");//anterior $neto
	$pdf->Text(197,$y_com,"$ $valor_real");// anterior $diff
	$pdf->SetFont('Arial','',8);
	$y_com=$y_com+5;
    $res_comp->MoveNext();
};
