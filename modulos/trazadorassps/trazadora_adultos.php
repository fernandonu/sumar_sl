...
if ($_POST['guardar']=="Guardar Planilla"){
   ...
   $fecha_control_trz_adultos=Fecha_db($_POST['fecha_control_trz_adultos']); 
   $peso=$_POST['peso'];
   $talla=$_POST['talla'];
   if ($talla!=0) $imc=($peso/($talla*$talla));
    else $imc=0;
   ...
   $comentario_trz_adultos=$_POST['observaciones_trz_adultos'];
   ...
   $fecha_nac_trz_adultos=$res_benef->fields['afifechanac'];
   $sexo=trim($res_benef->fields['afisexo']);
   $clavebeneficiarios=$res_benef->fields['clavebeneficiario'];
   $id_beneficiarios=0;
 }
 else {
  ...
  $fecha_nac_trz_adultos=$res_benef->fields['fecha_nac'];
...
$query="insert into trazadoras.adultos
		  ...
		  values
		  ('$id_planilla','$cuie','$clavebeneficiarios','P','DNI','$num_doc','$apellido','$nombre','$fecha_nac_trz_adultos',
			'$fecha_control_trz_adultos',$peso,$talla,$imc,
			'Desde Emite Prestacion','$fecha_carga','$usuario','$tension_arterial')";
    

sql($query, "Error al insertar la Planilla en la trazadora Adultos") or fin_pagina();
$accion="Se guardo la Planilla en Trazadora Adultos";    
...
if (($pagina=='prestacion_admin.php')&&($pagina_viene!="comprobante_admin_total.php")){
  ...
  $sql="select * from nacer.smiafiliados	  
	...
	$clavebeneficiarios=$res_extra->fields['clavebeneficiario'];
  ...
  $nombre=$res_extra->fields['nombre'];
	...
	$fecha_control_trz_adultos=$fecha_comprobante;
...
{ 
 ...
 if(document.all.fecha_control_trz_adultos.value==""){
...
if (document.all.peso.value==""){ 
  ...
  if (document.all.peso.value>250) {
    alert ('El Peso debe ser menor de 250 Kg');
    document.all.peso.focus();
    return false;
    }
    
  if (document.all.peso.value<=40) {
    alert ('El Peso debe ser mayor de 40 Kg');
    document.all.peso.focus();
    return false;
    }
...
if(document.all.talla.value==""){
   ...
   } else   if(document.all.talla.value >= 220  ){
    alert('la talla no puede superar los 220 Cm');
    document.all.talla.focus();
    return false;
   
   } else  if(document.all.talla.value<=110  ){
    alert('la talla no puede ser menor 110 Cm');
    document.all.talla.focus();
    return false;
   }
...
<form name='form1' action='trazadora_adultos.php' method='POST'>
       ...
       <td>
        ...
        <tr>	           
         ...
         <tr>
            ...
            <td align='left'>
              <input type="text" size="40" value="<?=$clavebeneficiarios?>" name="clave" <? if ($id_planilla) echo "readonly"?>>
            </td>
         </tr> 
		...
		<tr>
		    ...
		    <td align="left">
		    	<?$fecha_control_trz_adultos=date("d/m/Y");?>
		    	 <input type=text id=fecha_control_trz_adultos name=fecha_control_trz_adultos value='<?=$fecha_control_trz_adultos;?>' size=15>
		    	 <?=link_calendario("fecha_control_trz_adultos");?>					    	 
		    </td>		    
		</tr>		
		 <tr>
           <td align="right">
          ...
          <tr>
           ...
           <td >
             <input type='text' name='talla' value='<?=$talla;?>' size=60 align='right'></b><font color="Red">Mts (Metros)</font>
           </td>
          </tr>
          <tr>
           <td align="right">
		 ...
		 <tr>
            ...
            <td align='left'>
              <textarea cols='40' rows='4' name='observaciones_trz_adultos' <? if ($id_planilla) echo "readonly"?>><?=$observaciones_trz_adultos;?></textarea>
            </td>
         </tr>            
        </table>
      </td>      
...
<?}?>
  ...
  <tr align="center">
   <td>
   	<font color="Black" size="3"> <b>En esta pantalla se registran los datos de la Trazadora de Adultos</b></font>
   </td>
  </tr>
  <tr align="left">
   <td>
   	<font size="2">Trazadora Adultos.</font>
   </td>
  </tr>
