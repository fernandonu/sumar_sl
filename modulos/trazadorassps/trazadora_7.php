...
$edad_con_meses=edad_con_meses($fecha_nac_trz7);
  ...
  $dias=$edad_con_meses['dias'];
    ...
    $percentilo_peso_edad_trz7=calculo_percentilo_peso($dias,$sexo,$peso);
    $percentilo_talla_edad_trz7=calculo_percentilo_talla($dias,$sexo,($talla*100));
  ...
  if ($anio>5 and $anio<=19) {
    $meses=$meses+($anio*12);
    $percen_imc_edad=calculo_percentilo_imc($meses,$sexo,$imc);
    $percentilo_talla_edad_trz7=calculo_percentilo_talla($meses,$sexo,($talla*100));
   ...
   $query="INSERT into trazadorassps.trazadora_7	
             ...
             ,fecha_carga,usuario,comentario,percen_imc_edad,imc)
             values
             ('$id_planilla','$cuie','$id_smiafiliados','$id_beneficiarios','$fecha_nac_trz7','$fecha_control_trz7','$peso','$talla',
               ...
               '$fecha_carga','$usuario','$comentario_trz7','$percen_imc_edad',$imc)";
...
if (document.all.peso.value==""){ 
  ...
  if (document.all.peso.value<1.5) {
    alert ('El Peso debe ser menor de 1.5 Kg');
    document.all.peso.focus();
    return false;
    }
...
if(document.all.talla.value==""){
   ...
   } else   if(document.all.talla.value >= 180  ){
    alert('la talla no puede superar los 180 Cm');
    document.all.talla.focus();
    return false;
   
   } else  if(document.all.talla.value<=40  ){
    alert('la talla no puede ser menor a 40 Cm');
    document.all.talla.focus();
    return false;
   }
