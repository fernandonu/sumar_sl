...
if ($_POST['guardar']=="Guardar Planilla"){
   ...
   $query="insert into trazadorassps.trazadora_11	
             ...
             '$fecha_carga','$usuario','$comentario_trz11',$id_beneficiarios)";
       ...
       <td>
		...
		<tr>
		 ...
		 <tr>
           ...
           <td align="left">
           <?$q="SELECT * from facturacion.nomenclador where id_nomeclador_detalle=12 and grupo='TA'";
             $res_q=sql($q) or fin_pagina();?>
          <select name=tema_taller Style="width=600px">
          <option value=-1>Seleccione</option>
        		<?while (!$res_q->EOF){
              $codigo_ta=$res_q->fields['codigo'];
              $desc=$res_q->fields['descripcion'];?>
            <option value="<?=$codigo_ta?>"><?=$codigo_ta.' - '.$desc?></option>
        		<?$res_q->movenext();
            }?>	 
			</select>
			</td>
