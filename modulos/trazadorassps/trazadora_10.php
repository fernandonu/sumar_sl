...
if ($_POST['guardar']=="Guardar Planilla"){
   ...
   if ($talla_trz10!=0) $imc=($peso_trz10/($talla_trz10*$talla_trz10));
    else $imc=0;
  ...
  $edad_con_meses=edad_con_meses($fecha_nac_trz10);
  ...
  if ($anio>5 and $anio<=19) {
    ...
    $percentilo_talla_edad=calculo_percentilo_talla($meses,$sexo,($talla*100));

    } 
  else  {
    $percen_imc_edad="Datos No Ingresados";
    $percentilo_talla_edad="Datos No Ingresados";    
  } ; 
   ...
   $id_planilla=$id_planilla->fields['id_planilla'];
    ...
    $query="insert into trazadorassps.trazadora_10	
             ...
             ,fecha_carga,usuario,comentario,percen_imc_edad,percentilo_talla_edad,imc)
             values
             ...
             '$fecha_carga','$usuario','$comentario_trz10','$percen_imc_edad','$percentilo_talla_edad',$imc)";
...
if (document.all.peso_trz10.value==""){ 
  ...
  if (document.all.peso_trz10.value>250) {
    alert ('El Peso debe ser mayor de 250 Kg');
    document.all.peso_trz10.focus();
    return false;
    }
    
  if (document.all.peso_trz10.value<=15) {
    alert ('El Peso debe ser mayor de 15 Kg');
    document.all.peso_trz10.focus();
    return false;
    }
...
if(document.all.talla_trz10.value==""){
    alert('Debe ingresar la talla');
      document.all.talla_trz10.focus();
    return false;
  
  } else   if(isNaN(document.all.talla_trz10.value)){
    alert('la talla Debe ser un Numero Real');
    document.all.talla_trz10.focus();
    return false;
      
   } else   if(document.all.talla_trz10.value >= 220  ){
    alert('la talla no puede superar los 220 Cm');
    document.all.talla_trz10.focus();
    return false;
   
   } else  if(document.all.talla_trz10.value<=110  ){
    alert('la talla no puede ser 0 o superar los 110 Cm');
    document.all.talla_trz10.focus();
    return false;
   }
