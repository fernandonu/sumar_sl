...
if ($_POST['muestra']=="Muestra"){
	...
	if (fecha_db($fecha_desde)>='2015-01-01') {	
	$link=encode_link("listado_factura_dep_pcia.php",array("cuie"=>$cuie,"fecha_desde"=>$fecha_desde,"fecha_hasta"=>$fecha_hasta));
	}
	else {$link=encode_link("listado_factura_dep_pcia_2014.php",array("cuie"=>$cuie,"fecha_desde"=>$fecha_desde,"fecha_hasta"=>$fecha_hasta));}
	?>
	<script>
	window.open('<?php echo $link?>')
	</script>
...
if (($_POST['Filtrar']=="Filtrar")||($_POST['generar_excel']=="Generar Excel")){
	...
	$date = DateTime::createFromFormat('Y-m-d', $fecha_desde);
	$date_backups = DateTime::createFromFormat('Y-m-d', '2015-09-01');

if ($date>=$date_backups) {
				...
				FROM
					...
					WHERE id_factura in 
						(SELECT DISTINCT
							factura.id_factura
						FROM facturacion.factura 
						INNER JOIN expediente.expediente_backup ON facturacion.factura.id_factura = expediente.expediente_backup.id_factura
						INNER JOIN expediente.transaccion_backup ON expediente.transaccion_backup.id_expediente = expediente.expediente_backup.id_expediente
						WHERE
							(expediente.transaccion_backup.id_area = 1 AND expediente.transaccion_backup.estado = 'D') AND
							(expediente.transaccion_backup.num_tranf is null) AND
							(expediente.transaccion_backup.fecha_mov BETWEEN '$fecha_desde' AND '$fecha_hasta') 
						))as cons1
				INNER JOIN facturacion.comprobante USING (id_factura)
				INNER JOIN facturacion.prestacion USING (id_comprobante)
				INNER JOIN facturacion.nomenclador USING (id_nomenclador)
      ...
      <td align=center>
		<b>Periodo para DDJJ:	        			
			 <select name=periodo Style="width=100px">&nbsp;
			  ...
			  while (!$result1->EOF) {?>	  
			  <option value=<?php echo $result1->fields['periodo']?> <?if ($result1->fields['periodo']==$_POST['periodo']) echo "selected"?>><?php echo $result1->fields['periodo']?></option>
			  <?php 
			  $result1->movenext();
			  }
			  ?>			  
	    	  ...
	    	  <input type=button name="ddjj_por_periodo" value="DDJJ Por Periodo" onclick="window.open('<?php echo encode_link("../facturacion/listado_factura_dep_periodo.php",array())?>','DDJJ','dependent:yes,width=1000,height=700,top=1,left=60,scrollbars=yes');" title="DDJJ" >
...
<table border=0 width=100% cellspacing=2 cellpadding=2 bgcolor='<?php echo $bgcolor3?>' align=center>
  <tr>
     ...
     <table width=100%>
      <tr id=ma>
       <td width=30% align=left><b>Total:</b> <?php echo $result->recordCount()?> Filtro: <?php echo $_POST['periodo'];?></td>       
      </tr>
    </table>
   </td>
  </tr>
 ...
 <? $t=0;
   ...
   while (!$result->EOF) {  	
    ...
    <tr <?php echo atrib_tr()?> >     
     <td bgcolor="<?php echo $color_catas?>" title="<?php echo $title_catas?>"><?php echo $result->fields['codigo']?></td>     
     <td bgcolor="<?php echo $color_priori?>" title="<?php echo $title_priori?>"><?php echo $result->fields['descripcion']?></td>     
     <td><?php echo $result->fields['cantidad']?></td>     
     <td><?php echo number_format($result->fields['precio_prestacion'],2,',','.')?></td>     
     <td><?php echo number_format($result->fields['precio_total'],2,',','.')?></td>   
    ...
    <tr>    	  
     	<td>Monto Total</td> 
     	<td><b><?php echo number_format($t,2,',','.')?></b></td> 
    </tr>
    <tr>    	
     	<td>Total Prestaciones</td> 
     	<td><b><?php echo number_format($s,0,',','.')?></b></td> 
    </tr> 
...
<?php if ($_POST['Filtrar']=="Filtrar"){
$result = sql($sql_fact) or die;?>

<table border=0 width=70% cellspacing=2 cellpadding=2 bgcolor='<?php echo $bgcolor3?>' align=center>
  <tr>
  	<td colspan=10 align=left id=ma>
     <table width=80%>
      <tr id=ma>
 ...
 <?php $t=0;
   ...
   while (!$result->EOF) {
    ...
    <tr <?php echo atrib_tr()?> >
     ...
     <td bgcolor="<?php echo $color_1?>" title="<?php echo $title_1?>"><a href="<?php echo $ref?>" target="_blank"><?php echo $result->fields['id_factura']?></a></td>     
     <td bgcolor="<?php echo $color_1?>" title="<?php echo $title_1?>"><?php echo $result->fields['cuie']?></td>     
     <td bgcolor="<?php echo $color_1?>" title="<?php echo $title_1?>"><?php echo fecha($result->fields['fecha_carga'])?></td>     
     <td bgcolor="<?php echo $color_1?>" title="<?php echo $title_1?>"><?php echo fecha($result->fields['fecha_factura'])?></td>       
    </tr>
	<?$result->MoveNext();
   }?>   
</table>
