...
$filename = 'SUMAR-SIRGE-12-COMPROBANTES.txt';	
    	...
    	while (!$result1->EOF) {
			...
			$sql_res=sql($sql_mod,"No se pudo actualizar el estado de la factura") or fin_pagina();
...
//------------------------------------------------------------------------------------------------------------------------
    	...
    	fclose($handle);   	
//------------------------------------------------------------------------------------------------------------------------   
//					PRESTACIONES PARA SISTEMA SIRGEWEB (CON ACTUALIZACION DE ESTADO DE ENVIO)
//------------------------------------------------------------------------------------------------------------------------
    	...
    	$sql1="SELECT tabla.id_prestacion,tabla.operacion,tabla.estado,tabla.numero_comprobante,tabla.subcodigo_prestacion,tabla.precio_unitario,tabla.fecha_prestacion,tabla.clave_beneficiario,tabla.tipo_documento,
			tabla.clase_documento,tabla.numero_documento,tabla.afifechanac,tabla.id_smiafiliados,
			--tabla.peso,tabla.tension_arterial,tabla.talla,tabla.perim_cefalico,tabla.edad_gestacional,
			tabla.retinopatia,
			tabla.inf_anat_patologica,tabla.inf_diag_biopsia,tabla.inf_vdrl,tabla.res_oido_derecho,tabla.res_oido_izquierdo,inf_diag_anatomo,tratamiento_instaurado,cpod,ceod,
			tabla.efector,
			...
			SELECT prestacion.id_prestacion,
				'A' as operacion,
				'L' as estado,
				facturacion.comprobante.id_factura as numero_comprobante,
				facturacion.anexo.numero as subcodigo_prestacion, --CUANDO DEVUELVE '0' EN EL ARCHIVO VA VACIO
			  ...
			  facturacion.prestacion.retinopatia,
			  facturacion.prestacion.inf_anat_patologica,
			  facturacion.prestacion.inf_diag_biopsia,
			  facturacion.prestacion.inf_vdrl,
			  facturacion.prestacion.res_oido_derecho,
			  facturacion.prestacion.res_oido_izquierdo,
			  facturacion.prestacion.inf_diag_anatomo,
			  facturacion.prestacion.tratamiento_instaurado,
			  facturacion.prestacion.cpod,
			  facturacion.prestacion.ceod,
			...
			CASE WHEN facturacion.nomenclador_detalle.modo_facturacion='1' THEN facturacion.nomenclador.codigo
			    WHEN facturacion.nomenclador_detalle.modo_facturacion<>'1' THEN facturacion.nomenclador.grupo||facturacion.nomenclador.codigo||facturacion.prestacion.diagnostico
			    END as codigo_prestacion,
			facturacion.prestacion.estado_envio
			FROM
			facturacion.prestacion
			...
			left JOIN nacer.smiafiliados ON (facturacion.comprobante.id_smiafiliados = nacer.smiafiliados.id_smiafiliados)
			left JOIN facturacion.factura ON (comprobante.id_factura = factura.id_factura)
			--left JOIN trazadoras.adultos ON (round (adultos.num_doc)::text=nacer.smiafiliados.afidni)
			--left JOIN fichero.fichero on nacer.smiafiliados.id_smiafiliados=fichero.fichero.id_smiafiliados
			INNER JOIN contabilidad.ingreso ON (facturacion.factura.id_factura = contabilidad.ingreso.numero_factura)
			left join facturacion.anexo on (prestacion.id_anexo = anexo.id_anexo)
			Where
			...
			and (facturacion.prestacion.estado_envio is null or facturacion.prestacion.estado_envio='n')
    	...
    	while (!$result1->EOF) {    		
			$edad=edad_con_meses_sin_fecha_actual($result1->fields['afifechanac'],$result1->fields['fecha_prestacion']);
			$edad=$edad["anos"];
			$dni=$result1->fields['numero_documento'];
			$id_smiafiliados=$result1->fields['id_smiafiliados'];
			$fecha_control=$result1->fields['fecha_prestacion'];
			...
			$contenido.=$result1->fields['numero_comprobante'];
			$contenido.=";"; 
			$codigo=str_replace(" ", "",$result1->fields['codigo_prestacion']);
      		$contenido.=$codigo.";";
			if ($result1->fields['subcodigo_prestacion']<>0) $contenido.=$result1->fields['subcodigo_prestacion'];
			$contenido.=";"; 
			$contenido.=$result1->fields['precio_unitario'];
      ...
      if (in_array($codigo, $codigos_embarazadas) and $id_smiafiliados){
        	$q="SELECT *,round(edad_gestacional) as fix_edad_gestacional from trazadorassps.trazadora_1 where id_smiafiliados=$id_smiafiliados and (fecha_control_prenatal='$fecha_control' or fecha_carga='$fecha_control')";
	        $res_q=sql($q) or fin_pagina();
	        if ($res_q->recordcount()>0) {        
		        $contenido.="1;";
		        $contenido.=str_replace('.',',',$res_q->fields['peso']).";";
		        $contenido.="5;";
		        $contenido.=$res_q->fields['fix_edad_gestacional'].";";
		        $contenido.="3;";
		        $contenido.=$res_q->fields['ta'].";";
		        $contenido.=";;";//dos lugares vacios para completar la linea son 4 datos reportables       
	      	}
	      		else {
	  			$q="SELECT *,round(edad_gestacional) as fix_edad_gestacional from trazadorassps.trazadora_2 where id_smiafiliados=$id_smiafiliados and (fecha_control='$fecha_control' or fecha_control='$fecha_control')";
	  			$res_q=sql($q) or fin_pagina();
	  			if ($res_q->recordcount()>0) {
		  			$contenido.="1;";
			        $contenido.=str_replace('.',',',$res_q->fields['peso']).";";
			        ...
			        $contenido.=$res_q->fields['fix_edad_gestacional'].";";
			        $contenido.="3;";
			        $contenido.=$res_q->fields['tension_arterial'].";";
			        $contenido.=";;";//dos lugares vacios para completar la linea son 4 datos reportables
			        }

			     else {
			     	if ($id_smiafiliados) {
						$r="SELECT * from fichero.fichero where id_smiafiliados=$id_smiafiliados and fecha_control='$fecha_control'";
						$res_r=sql($r) or fin_pagina();
						$contenido.="1;";
						$contenido.=str_replace('.',',',$res_q->fields['peso']).";";
						$contenido.="5;";
						$contenido.=$res_q->fields['semana_gestacional'].";";
						$contenido.="3;";
						$contenido.=$res_q->fields['ta'].";";
						$contenido.=";;";//dos lugares vacios para completar la linea son 4 datos reportables
					}
      ...
      elseif (in_array($codigo, $codigo_obes_sobrep) and $id_smiafiliados ) {
      	$q="SELECT * from fichero.fichero where id_smiafiliados=$id_smiafiliados and fecha_control='$fecha_control'";
        $res_q=sql($q) or fin_pagina();
        if ($res_q->RecordCount()>0) {
        	$contenido.="1;";
	        $contenido.=str_replace('.',',',$res_q->fields['peso']).";";
	        $contenido.="2;";
	        if ($res_q->fields['talla']<=2.5) $fix_talla_cm=round($res_q->fields['talla']*100);
	        	else $fix_talla_cm=round($res_q->fields['talla']);
	        $contenido.=$fix_talla_cm.";";
	        $contenido.="3;";
	        $contenido.=$res_q->fields['ta'].";";	
        }
      ...
      elseif (in_array($codigo, $codigo_ninios_adolesc) and $id_smiafiliados) {
        $q="SELECT * from fichero.fichero where id_smiafiliados=$id_smiafiliados and fecha_control='$fecha_control'";
        $res_q=sql($q) or fin_pagina();
        if ($res_q->RecordCount()>0) {
	        $contenido.="1;";
	        ...
	        $contenido.=$res_q->fields['ta'].";";
	        $contenido.="4;";
	        $contenido.=str_replace('.',',',$res_q->fields['perim_cefalico']).";";
	    }
	    else {
	    	$q="SELECT * from trazadoras.nino_new where round(num_doc)='$dni' and fecha_control='$fecha_control'";
	    	$res_q=sql($q) or fin_pagina();
	    	if ($res_q->RecordCount()>0){
	    		$contenido.="1;";
		        $contenido.=$res_q->fields['peso'].";";
		        $contenido.="2;";
		        if ($res_q->fields['talla']<=2.5) $fix_talla_cm=round($res_q->fields['talla']*100);
	        		else $fix_talla_cm=round($res_q->fields['talla']);
	        	$contenido.=$fix_talla_cm.";";
	    		...
	    		$q="SELECT * from trazadoras.adultos where round(num_doc)='$dni' and fecha_control='$fecha_control'";
	    		$res_q=sql($q) or fin_pagina();
	    		if ($res_q->RecordCount()>0) {
		    		$contenido.="1;";
			        $contenido.=str_replace('.',',',$res_q->fields['peso']).";";
			    	...
			    	$r="SELECT * from trazadorassps.trazadora_7 where id_smiafiliados=$id_smiafiliados and fecha_control='$fecha_control'";
		            $res_r=sql($r) or fin_pagina();
		            if ($res_r->RecordCount()>0){
		            	...
		            	$r="SELECT * from trazadorassps.trazadora_10 where id_smiafiliados=id_smiafiliados and fecha_control='$fecha_control'";
              			$res_r=sql($r) or fin_pagina();
              			$contenido.="1;";
				        $contenido.=str_replace('.',',',$res_q->fields['peso']).";";
				        $contenido.="2;";
      ...
      elseif ($codigo=="CTC010W78") {
        $contenido.="6;";
        $contenido.=$result1->fields['cpod'].";";
        $contenido.=";;;;;;";
      }

      elseif ($codigo=="CTC010A97") {
        $contenido.="6;";
        if ($edad>0 and $edad <6) {
        		$contenido.=$result1->fields['ceod'].";";
        		$contenido.=";;;;;;";
        		}
        else {
        		$contenido.=$result1->fields['cpod'].";";
      ...
      elseif ($codigo=="PRP021A97" or $codigo=="PRP021H86") {
        $contenido.="7;";
        $contenido.=$result1->fields['res_oido_derecho'].";";
        $contenido.="7;";
        $contenido.=$result1->fields['res_oido_izquierdo'].";";
        $contenido.=";;;;";
      }
      
      elseif ($codigo=="PRP017A46" or $codigo=="PRP017A97") {
        $contenido.="8;";
        $contenido.=$result1->fields['retinopatia'].";";
        $contenido.=";;;;;;";
      }
      
      elseif ($codigo=="APA002X76") {
        $contenido.="9;";
        $contenido.=$result1->fields['inf_diag_biopsia'].";";
        $contenido.=";;;;;;";
      }

      elseif ($codigo=="APA002A98" or $codigo=="APA002X75" or $codigo=="APA002X80") {
        $contenido.="10;";
        $contenido.=$result1->fields['inf_anat_patologica'].";";
        $contenido.=";;;;;;";
      }
      
      elseif ($codigo=="APA001A98" or $codigo=="APA001X86" or $codigo=="APA001X75") {
        $contenido.="11;";
        $contenido.=$result1->fields['inf_diag_anatomo'].";";
        $contenido.=";;;;;;";
      }


      elseif ($codigo=="LBL119A97" or $codigo=="LBL119W78") {
        $contenido.="13;";
        $contenido.=$result1->fields['inf_vdrl'].";";
        $contenido.=";;;;;;";
      }

      elseif ($codigo=="NTN002X75") {
        $contenido.="14;";
        $contenido.=$result1->fields['tratamiento_instaurado'].";";
        $contenido.=";;;;;;";
      }

      elseif ($codigo_comp=="CTC010A97" or $codigo_comp=="CTC010w78") {
        $contenido.="6;";
        if ($edad<=12) $contenido.=$res_fact->fields['ceod'];
          else $contenido.=$res_fact->fields['cpod'];
        $contenido.=";;;;;;";
      }
      ...
      /*$contenido.=$result1->fields['id_dato_reportable'];
			...
			$sql_res_update2=sql($sql_update2,"No se pudo actualizar facturacion.prestacion") or fin_pagina();
