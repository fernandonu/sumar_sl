	...
	$sql_tmp=" SELECT			
			...
			FROM
				...
				WHERE id_factura in 
					...
					WHERE
						(expediente.transaccion_backup.estado = 'D') AND
						(expediente.transaccion_backup.num_tranf = 0 or expediente.transaccion_backup.num_tranf is null) AND
...
$sql_tmp=" SELECT			
			...
			FROM
				...
				WHERE id_factura in 
					...
					WHERE
						(expediente.transaccion.estado = 'D') 
						--AND (expediente.transaccion.num_tranf = 0) 
						AND (expediente.transaccion.fecha_mov BETWEEN '$fecha_desde' AND '$fecha_hasta') 
					))as cons1
			INNER JOIN facturacion.comprobante USING (id_factura)
			INNER JOIN facturacion.prestacion USING (id_comprobante)
			INNER JOIN facturacion.nomenclador USING (id_nomenclador)
