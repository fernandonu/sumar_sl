...
$codigos_datos_rep = array ("CTC005W78","CTC006W78","CTC007O24.4","CTC022O24.4","CTC007O10","CTC007O10.4",
                            "CTC022O10","CTC022O10.4","CTC007O16","CTC022O16","TC017P05","CTC001A97",
							"CTC009A97","CTC001T79","CTC001T82","CTC001T83","CTC002T79","CTC002T82","CTC002T83",
							"CTC010A97","CTC010W78");
...
/*function busca_datos_reportables ($id_comprobante,$codigo_comp){
	...
	$codigo_odonto = array ("CTC010A97","CTC010W78");


	$query_comp = "SELECT * from facturacion.comprobante a, facturacion.prestacion b
									WHERE a.id_comprobante = b.id_comprobante
									AND a.id_comprobante = $id_comprobante";
	$res_query_comp = sql ($query_comp) or fin_pagina();

	$id_smiafiliados = $res_query_comp->fields['id_smiafiliados'];
	$fecha_comp = $res_query_comp->fields['fecha_comprobante'];
	
	$afi = "SELECT * from nacer.smiafiliados 
					where id_smiafiliados = $id_smiafiliados";
	$res_afi = sql($afi) or fin_pagina();
	$dni = $res_afi->fields['afidni'];
	$fecha_nac = $res_afi->fields['afifechanac'];
  ...
  $anios=$edad['anos'];
	...
	//datos reportables
	if (in_array($codigo_comp, $codigos_embarazadas)) {
	    ...
	    if ($res_r->RecordCount()>0) {
	      $dr = array (
						'peso' =>$res_r->fields['peso'],
						'ta' =>$res_r->fields['ta'],
						'eg' =>$res_r->fields['edad_gestacional'],
						'percef'=>$res_query->fields['perim_cefalico']*100
					);
			return $dr;
	    }

	    else {
	      $r="SELECT * from trazadorassps.trazadora_2 where id_smiafiliados=$id_smiafiliados and (fecha_control='$fecha_comp' or fecha_carga='$fecha_comp')";
	      $res_r=sql($r) or fin_pagina();
	      if ($res_r->RecordCount()>0) {
	        $dr = array (
						'peso' =>$res_r->fields['peso'],
	        ...
	        $r="SELECT * from fichero.fichero where id_smiafiliados=$id_smiafiliados and fecha_control='$fecha_comp'";
	        $res_r=sql($r) or fin_pagina();
	        $dr = array (
						'peso' =>($res_r->fields['peso'])?$res_r->fields['peso']:'Sin Datos',
						'ta' =>($res_r->fields['ta'])?$res_r->fields['ta']:'Sin Datos',
						'eg' =>($res_r->fields['edad_gestacional'])?$res_r->fields['edad_gestacional']:'Sin Datos',
						'percef'=>$res_query->fields['perim_cefalico']*100
					);
					return $dr;
	      }
	...
	elseif (in_array($codigo_comp, $codigo_ninios_adolesc)) {
    
    $r="SELECT * from trazadoras.nino_new where round(num_doc)='$dni' and fecha_control='$fecha_comp'";
    $res_r=sql($r) or fin_pagina();
    if ($res_r->RecordCount()>0){
      if ($res_r->fields['talla']<=2.5) $fix_talla_cm=round($res_r->fields['talla']*100);
        else $fix_talla_cm=round($res_r->fields['talla']);
      $dr = array (
					'peso' =>($res_r->fields['peso'])?$res_r->fields['peso']:'Sin Datos',
					'talla' => $res_r->fields['talla'],
					'ta' =>($res_r->fields['ta'])?$res_r->fields['ta']:'Sin Datos',
					'eg' =>($res_r->fields['edad_gestacional'])?$res_r->fields['edad_gestacional']:'Sin Datos',
					'percef'=>($res_r->fields['perim_cefalico']<=1) ? $res_r->fields['perim_cefalico']*100 : $res_r->fields['perim_cefalico']
				);
			return $dr;
        ...
        $r="SELECT * from trazadoras.adultos where round(num_doc)='$dni' and fecha_control='$fecha_comp'";
        $res_r=sql($r) or fin_pagina();
        if ($res_r->RecordCount()>0) {
          if ($res_r->fields['talla']<=2.5) $fix_talla_cm=round($res_r->fields['talla']*100);
            else $fix_talla_cm=round($res_r->fields['talla']);
          ...
          $r="SELECT * from trazadorassps.trazadora_7 where id_smiafiliados=id_smiafiliados and fecha_control='$fecha_comp'";
        $res_r=sql($r) or fin_pagina();
        if ($res_r->RecordCount()>0){
          if ($res_r->fields['talla']<=2.5) $fix_talla_cm=round($res_r->fields['talla']*100);
            else $fix_talla_cm=round($res_r->fields['talla']);
          $dr = array (
						'peso' =>($res_r->fields['peso'])?$res_r->fields['peso']:'Sin Datos',
						'talla' => $fix_talla_cm,
						'ta' =>($res_r->fields['tension_arterial'])?$res_r->fields['tension_arterial']:'Sin Datos',
						'eg' =>($res_r->fields['edad_gestacional'])?$res_r->fields['edad_gestacional']:'Sin Datos',
						'percef'=>($res_r->fields['perim_cefalico']<=1) ? $res_r->fields['perim_cefalico']*100 : $res_r->fields['perim_cefalico']
					);
					return $dr;
          }
        else {$r="SELECT * from trazadorassps.trazadora_10 where id_smiafiliados=id_smiafiliados and fecha_control='$fecha_comp'";
          $res_r=sql($r) or fin_pagina();
          if ($res_r->fields['talla']<=2.5) $fix_talla_cm=round($res_r->fields['talla']*100);
            else $fix_talla_cm=round($res_r->fields['talla']);
          $dr = array (
  ...
  } // del control de general

  elseif (in_array($codigo_comp, $codigo_obes_sobrep)) {

      $q="SELECT * from fichero.fichero where id_smiafiliados=$id_smiafiliados and fecha_control='$fecha_comp'";
      $res_q=sql($q) or fin_pagina();
      if ($res_q->RecordCount()>0) {
        if ($res_r->fields['talla']<=2.5) $fix_talla_cm=round($res_r->fields['talla']*100);
          else $fix_talla_cm=round($res_r->fields['talla']);
        $dr = array (
						'peso' =>($res_r->fields['peso'])?$res_r->fields['peso']:'Sin Datos',
        ...
        $q="SELECT * from trazadorassps.trazadora_7 where id_smiafiliados=$id_smiafiliados 
          and fecha_control='$fecha_comp'";
        $res_q=sql($q) or fin_pagina();
        if ($res_q->RecordCount()>0) {
          if ($res_r->fields['talla']<=2.5) $fix_talla_cm=round($res_r->fields['talla']*100);
            else $fix_talla_cm=round($res_r->fields['talla']);
        ...
        $q="SELECT * from trazadorassps.trazadora_10 where id_smiafiliados=$id_smiafiliados 
            and fecha_control='$fecha_comp'";
          $res_q=sql($q) or fin_pagina(); 
          if ($res_r->fields['talla']<=2.5) $fix_talla_cm=round($res_r->fields['talla']*100);
            else $fix_talla_cm=round($res_r->fields['talla']);
    ...
    }//del control de sobrepeso

  if (in_array($codigo_comp, $codigo_odonto)) {
  	if ($anios<=12) $dato_odo1=($res_query_comp->fields['ceod']=='' or $res_query_comp->fields['ceod']=='c:/ e:/ o:') ? 'c:00/e:00/o:00' : $res_query_comp->fields['ceod'];
        else $dato_odo2=($res_query_comp->fields['cpod']=='' or $res_query_comp->fields['cpod']=='C:/ P:/ O:') ? 'C:00/P:00/O:00' : $res_query_comp->fields['cpod'];
      $dr = array (
						'ceod_cpod' => ($dato_odo1)?$dato_odo1:$dato_odo2
					);
					return $dr;  
  }	
}//de la funcion
...
function busca_datos_reportables ($id_prestacion,$codigo_comp){

$codigos_pres=array ("CTC005W78","CTC006W78","CTC007O24.4","CTC022O24.4","CTC007O10","CTC007O10.4",
	"CTC022O10","CTC022O10.4","CTC007O16","CTC022O16","CTC017P05","CTC001A97","CTC009A97",
	"CTC001T79","CTC001T82","CTC001T83","CTC002T79","CTC002T82","CTC002T83","CTC010A97","CTC010W78",
	"CTC074K86",//cronicos
	"LBL056T90","LBL056T89",//hemoglobina Glicosilada
	"LBL119A41","LBL119A97","LBL119A98","LBL119W78","LBL119X70","LBL119Y70" //VDRL
	);	

/*$query_comp = "SELECT * from facturacion.comprobante a, 
	facturacion.prestacion b,
	nacer.smiafiliados c
	WHERE a.id_comprobante = b.id_comprobante
	AND a.id_smiafiliados = c.id_smiafiliados
	--AND a.id_comprobante = $id_comprobante
	and b.id_prestacion=$id_prestacion";*/

	$query_comp = "SELECT * from facturacion.prestacion b
				WHERE b.id_prestacion=$id_prestacion";

$res_query_comp = sql ($query_comp) or fin_pagina();

if (in_array($codigo_comp, $codigos_pres)
	and $res_query_comp->recordcount()>0) {
		$dr = array (
			'peso' =>$res_query_comp->fields['peso'],
			'talla' => $res_query_comp->fields['talla'],
			'ta' =>$res_query_comp->fields['tension_arterial'],
			'eg' =>$res_query_comp->fields['edad_gestacional'],
			'percef'=>$res_query_comp->fields['perim_cefalico'],
			'ceod' => $res_query_comp->fields['ceod'],
			'cpod' => $res_query_comp->fields['cpod'],
			'hemglic' => $res_query_comp->fields['hemo_glic'],
			'vdrl' => $res_query_comp->fields['inf_vdrl'],
			'tisomf' => $res_query_comp->fields['tisomf'],
			'vph' => $res_query_comp->fields['vph'],
			'inf_diag_anatomo' => $res_query_comp->fields['inf_diag_anatomo'], //informe de PAP
			'res_oido_derecho' => $res_query_comp->fields['res_oido_derecho'],
			'res_oido_izquierdo' => $res_query_comp->fields['res_oido_izquierdo']
		);
return $dr;
...
function borra_duplicados ($id_factura) {

	$q = "SELECT id_smiafiliados,afidni,fecha_comprobante,codigo,descripcion, count(*) as cantidad from facturacion.comprobante
	...
	if ($res_q->RecordCount()>0) {
		while (!$res_q->EOF) {
			$fecha_comprobante=$res_q->fields['fecha_comprobante'];
			$id_smiafiliados=$res_q->fields['id_smiafiliados'];
			$codigo=$res_q->fields['codigo'];

			$cons="SELECT distinct on (fecha_comprobante,id_smiafiliados,id_nomenclador) * from facturacion.comprobante
				inner join facturacion.prestacion using (id_comprobante)
				inner join facturacion.nomenclador using (id_nomenclador)
				where fecha_comprobante='$fecha_comprobante'
				and id_smiafiliados=$id_smiafiliados
				and codigo='$codigo'
				and id_factura=$id_factura";

			$res_cons=sql($cons) or fin_pagina();
			$id_comprobante=$res_cons->fields['id_comprobante'];
			$id_prestacion=$res_cons->fields['id_prestacion'];

			
			$delete="DELETE from facturacion.log_prestacion where id_prestacion=$id_prestacion";
			sql($delete) or fin_pagina();
			$delete="DELETE from facturacion.prestacion where id_prestacion=$id_prestacion";
			sql($delete) or fin_pagina();
...
function limpieza_tablas ($id_factura) {
...
if ($eliminar_pres=="True"){
	$db->StartTrans();	
	$query="DELETE FROM facturacion.log_prestacion where id_prestacion=$id_prestacion";
	sql($query, "Error al eliminar la log_prestacion") or fin_pagina();

	$query="DELETE from facturacion.prestacion where id_prestacion=$id_prestacion";
    sql($query, "Error al eliminar la prestacion") or fin_pagina();
    $accion="Se elimino la prestacion: $id_prestacion del comprobante Numero: $id_comprobante";  
    $db->CompleteTrans();
...
if ($_POST['abre_factura']=="Abre Factura"){
   ...
   $query="UPDATE facturacion.factura set estado='A'
   			where id_factura=$id_factura";
   sql($query, "Error al actualizar el estado de la factura") or fin_pagina();

   $query="DELETE FROM expediente.transaccion where id_factura=$id_factura";
   sql($query, "Error al borrar la factura desde el sistema de expediente.transaccion") or fin_pagina();

   $query="DELETE FROM expediente.expediente where id_factura=$id_factura";
   sql($query, "Error al borrar la factura desde el sistema de expediente.expediente") or fin_pagina();
	...
	if ($com_gestion_aux=='VERDADERO'){
		if (($fecha_comp_ges_aux<=$fecha_factura)&&($fecha_factura<=$fecha_fin_comp_ges_aux)){		 	
		   ...
		   $q="select nextval('facturacion.factura_id_factura_seq') as id_factura";
	    	...
	    	$accion="Error: La fecha de factura esta fuera de vigencia con el compromiso de gestion".$fecha_factura;
	    }
	}
	else{
...
if ($_POST['borrar_duplicados']=="Borrar Prest.Duplic."){

	borra_duplicados($id_factura);
	limpieza_tablas($id_factura);
}
...
//alert('El cierre de factura inicia el circuito de pago a travez del sistema de Expediente Esta seguro de Cerrar la factura: '+id_factura);
...
if (fecha_hoy>=fecha_final) {
  ...
  var r = confirm('El cierre de factura inicia el circuito de pago a travez del sistema de Expediente Esta seguro de Cerrar la facturaq?');
  return r;
  }
...
while (!$rs_sql_monto->EOF) {
  ...
  switch ($grupo) {
  	case 'A':{$monto_a=$rs_sql_monto->fields['subtotal'];break;};
  	case 'B':{$monto_b=$rs_sql_monto->fields['subtotal'];break;};
  	case 'C':{$monto_c=$rs_sql_monto->fields['subtotal'];break;};
  	case 'D':{$monto_d=$rs_sql_monto->fields['subtotal'];break;};
  	case 'E':{$monto_e=$rs_sql_monto->fields['subtotal'];break;};
  	case 'F':{$monto_f=$rs_sql_monto->fields['subtotal'];break;};
  	default :/*{$monto_vacio=$rs_sql_monto->fields['subtotal'];}*/break;
  	 };
...
<table width="95%" cellspacing=0 border=1 bordercolor=#E0E0E0 align="center" bgcolor='<?=$bgcolor_out?>' class="bordes">
 ...
 <tr><td>
  <table width=70% align="center" class="bordes">
    ...
    <!--detella de facturacion para discriminar hombres-->
   
    <tr id="mo">
      <td align="center" colspan="2">
        <b>Totales discriminados</b>
      </td>
    </tr>  

    <tr>
          <td align="left">
            <b>Monto Facturacion Grupo  A (Niños de 0 a 5 Años):</b>
          </td>           
          <td align='center'>
            <input type="text" name="monto_s_hombre" value="<?=number_format((($monto_a)?$monto_a:0),2,',','.')?>" style="width=250px" readonly>              
          </td>
    </tr>

    <tr>
          <td align="left">
            <b>Monto Facturacion Grupo  B (Niños de 6 a 9 Años):</b>
          </td>           
          <td align='center'>
            <input type="text" name="monto_s_hombre" value="<?=number_format((($monto_b)?$monto_b:0),2,',','.')?>" style="width=250px" readonly>              
          </td>
    </tr>

    <tr>
          <td align="left">
            <b>Monto Facturacion Grupo  C (Adolescentes):</b>
          </td>           
          <td align='center'>
            <input type="text" name="monto_s_hombre" value="<?=number_format((($monto_c)?$monto_c:0),2,',','.')?>" style="width=250px" readonly>              
          </td>
    </tr>

    <tr>
          <td align="left">
            <b>Monto Facturacion Grupo  D (Mujeres):</b>
          </td>           
          <td align='center'>
            <input type="text" name="monto_s_hombre" value="<?=number_format((($monto_d)?$monto_d:0),2,',','.')?>" style="width=250px" readonly>              
          </td>
    </tr>

    <tr>
          <td align="left">
            <b>Monto Facturacion Grupo  E (Hombres):</b>
          </td>           
          <td align='center'>
            <input type="text" name="monto_s_hombre" value="<?=number_format((($monto_e)?$monto_e:0),2,',','.')?>" style="width=250px" readonly>              
          </td>
    </tr>

    <tr>
          <td align="left">
            <b>Monto Facturacion Grupo  F (Adultos):</b>
          </td>           
          <td align='center'>
            <input type="text" name="monto_s_hombre" value="<?=number_format((($monto_f)?$monto_f:0),2,',','.')?>" style="width=250px" readonly>              
          </td>
    </tr>
    <!--
    <tr>
          <td align="left">
            <b>Monto Facturacion Grupo no Definido:</b>
          </td>           
          <td align='center'>
            <input type="text" name="monto_vacio" value="<?=number_format((($monto_vacio)?$monto_vacio:0),2,',','.')?>" style="width=250px" readonly>              
          </td>
    </tr>
    -->
   ...
   <?if ($estado=='A') {?>
	<tr id="mo">
  		<td align=center colspan="2">
  			<b><font color="White">Detalle Factura Abierta</font></b>
  		</td>
  	</tr>  
  	<tr>
	 <td align="center" colspan="2" class="bordes" bgcolor="#d3d3cd">		
	 	...
	 	&nbsp;&nbsp;
	 	<?$link=encode_link("detalle_factura.php", array("id_factura"=>$id_factura));	
		   echo "<br><a target='_blank' href='".$link."' title='Detalle Factura'><IMG src='$html_root/imagenes/logo_impresora.gif' height='35' width='35' border='0'></a>";?>
            ...
            ?>
	                  ...
	                  <table width=90% align=center class="table table-bordered">
								...
								<?}
										...
										//if ((in_array(($result_items->fields['grupo'].$result_items->fields['codigo'].$result_items->fields["cod_diag"]),$codigos_datos_rep))){
											if (1){	?>
		                           <tr id="ma">		                               
		                               <td>Cantidad</td>
		                               <td>Codigo</td>
		          ...
		          //if (in_array(($result_items->fields['grupo'].$result_items->fields['codigo'].$result_items->fields["cod_diag"]),$codigos_datos_rep))
		          //$res_comprobante->fields['id_comprobante']
		          $id_prestacion=$result_items->fields['id_prestacion'];
			  		$datos_reportables = busca_datos_reportables($id_prestacion,($result_items->fields['grupo'].$result_items->fields['codigo'].$result_items->fields["cod_diag"]));

		                            
		                            $ref2 = encode_link("factura_admin.php",array("id_prestacion"=>$id_prestacion,"id_comprobante"=>$res_comprobante->fields['id_comprobante'],"eliminar_pres"=>"True","id_factura"=>$id_factura));
		                            $onclick_eliminar="if (confirm('Esta Seguro que Desea Eliminar la Prestacion del Comprobante $id_comprobante_aux?, ESTA ACCION NO PUEDE SER REVERTIDA')) location.href='$ref2'
            						else return false;	";
		                            ?>
			                           <tr>
                        ...
                        <?//if (in_array(($result_items->fields['grupo'].$result_items->fields['codigo'].$result_items->fields["cod_diag"]),$codigos_datos_rep)) {
							if (1) {?>
	                                 <td align="center"><b><?php echo ($datos_reportables['peso']<>0)?number_format($datos_reportables['peso'],'2','.',''):null?></b></td>
	                                 <td align="center"><b><?php echo ($datos_reportables['talla']<>0)?number_format($datos_reportables['talla'],'2','.',''):null?></b></td>
	                                 <td align="center"><b><?php echo $datos_reportables['ta']?></b></td>
	                                 <td align="center"><b><?php echo ($datos_reportables['percef']<>0)?number_format($datos_reportables['percef'],'1','.',''):null?></b></td>
	                                 <td align="center"><b><?php echo $datos_reportables['eg']?></b></td>
	                                 <td align="center"><b><?php echo $datos_reportables['ceod']?></b></td>
									 <td align="center"><b><?php echo $datos_reportables['cpod']?></b></td>
									 <td align="center"><b><?php echo $datos_reportables['hemglic']?></b></td>
									 <td align="center"><b><?php echo $datos_reportables['vdrl']?></b></td>
