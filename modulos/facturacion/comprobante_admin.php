...
if (($_POST['guardar']=="Guardar Comprobante")||($_POST['guardar']=="Guardar Comprobante y Facturar")){
  ...
  if ($_POST['covid'] == "covid"){
	...
	if ($comienza_validacion=='S'){
	    ...
	    else $accion1="Ya se genero el comprobante Número $val_id_comp, para esta persona el mismo dia.";  
...
$q="SELECT 
      ...
      b.usuario
	FROM
      facturacion.comprobante a, facturacion.log_comprobante b
    where a.id_comprobante = b.id_comprobante 
    and a.id_smiafiliados=$id_smiafiliados --and comprobante.clavebeneficiario=$clavebeneficiario
...
<input type="hidden" name="clavebeneficiario" value="<?php echo $clavebeneficiario?>">
...
<input type="hidden" name="flag_inactivo" value="<?php echo $flag_inactivo?>">
<input type="hidden" name="fecha_ahora_js" value="<?php echo fecha(date("Y-m-d"));?>">
  ...
  <table width=70% align="center" class="bordes">
       ...
       <td>
        <table>
         ...
         <tr>
            ...
            <td align='left'>
              <input type='text' name='afiapellido' value='<?php echo $afiapellido;?>' size=60 align='right' readonly></b>
            </td>
         </tr>
         <tr>
            <td align="right">
           ...
           <td  colspan="2">
             <input type='text' name='afinombre' value='<?php echo $afinombre;?>' size=60 align='right' readonly></b>
           </td>
          </tr>
          <tr>
           <td align="right">
           ...
           <td colspan="2">
             <input type='text' name='afidni' value='<?php echo $afidni;?>' size=60 align='right' readonly></b>
           </td>
          </tr>
           <tr>
           <td align="right">
           ...
           <td colspan="2">
             <input type='text' name='afidni' value='<?php echo fecha($afifechanac);?>' size=60 align='right' readonly></b>
           </td>
          </tr>
          ...
          <tr>
           ...
           <td colspan="2">
             <input type='text' name='nombreefecto' value='<?php echo $nombre;?>' size=60 align='right' readonly></b>
           </td>
          </tr>
        </table>
      </td>      
	 ...
	 <table class="bordes" align="center" width="70%">
		 ...
		 <tr><td class="bordes"><table>
					    ...
					    <td align="left">		          			
							...
							<?$user_login1=substr($_ses_user['login'],0,6);
							  if (es_cuie($_ses_user['login'])){
							    ...
							    $sql= "select cuie, nombre, com_gestion,per_alta_com,covid from nacer.efe_conv 
							    	where conv_sumar=true order by nombre";
							 ...
							 $res_efectores=sql($sql) or fin_pagina();?>
			 				 ...
			 				 <?while (!$res_efectores->EOF){ 
							    ...
							    $ppac=$res_efectores->fields['per_alta_com'];
							    $covid=$res_efectores->fields['covid'];
								($ppac=='SI')?$color_style='#92F781':$color_style='';
								($covid=='SI')?$color_style='#F7E881':$color_style='';
							    ?>
								<option value=<?php echo $cuie;?> 
										Style="background-color: <?php echo $color_style?>;" 
										<?php if ($cuie==$efector) echo "selected"?>>
								<?php echo $cuie." - ".$nombre_efector?>
								</option>
							    <?
					 ...
					 <tr>
					    ...
					    <td align="left">		          			
			                 ...
			                 <? switch ($efector) { 
			                 	...
			                 	$res_efectores=sql($sql) or fin_pagina(); break;
			                 	...
			                 	$res_efectores=sql($sql) or fin_pagina(); break;
			                		...
			                		default : 
			                	$sql= "select * from facturacion.servicio  except 
										select * from facturacion.servicio where id_servicio=42 or id_servicio=51
			                 ...
			                 ?>
			                   <option <?php echo ($res_efectores->fields['descripcion']=="No Corresponde")?"selected":""?> value=<?php echo $id_servicio;?>><?php echo $descripcion?></option>
			                 <?
			                 $res_efectores->movenext();
			                }?>
			      			</select>
            ...
            <?$sql_flap="SELECT * 
                from nacer.efe_conv
                where cuie='$efector' and flap='s'";
            $res_efec_1_flap=sql($sql_flap,"no se pudo ejecutar");
                                   
            if ($res_efec_1_flap->recordcount()!=0) {?>
            <tr>
              <td align="right">
                <b>Factura ANOMALÍAS CONGÉNITAS:</b>
              </td>
              <td align="left">
                <input type="checkbox" name="flap" value="flap">               
              </td>       
            </tr>
            <?}?>
            ...
            <?$sql_covid="SELECT * 
                from nacer.efe_conv
                where cuie='$efector' and covid='SI'";
            $res_covid=sql($sql_covid,"no se pudo ejecutar la consulta de efectores por covid");
                                   
            if ($res_covid->recordcount()!=0) {?>
            <tr>
              <td align="right">
                <b>Factura COVID-19:</b>
              </td>
              <td align="left">
	 ...
	 <?}
	 	...
	 	while (!$res_comprobante->EOF) {
			...
			$title_1='PRESTACION ALTA COMPLEJIDAD';
      		...
      		elseif ($res_comprobante->fields['flap']=='s'){
      		$color_1='#9DF4AA'; 
      		$title_1='PRESTACION FLAP';
      		}
      		
      		elseif ($res_comprobante->fields['covid']=='s'){
      		$color_1='#F49DD6'; 
      		$title_1='PRESTACION COVID-19';
      		}
	 		...
	 		<tr <?php echo atrib_tr()?>>
		 		...
		 		<td onclick="<?php echo $onclick_elegir?>" bgcolor='<?php echo $color_fondo?>'><font size="3" color="Red"><b><?php echo $res_comprobante->fields['id_comprobante']. "(".$res_comprobante->fields['id_factura'].")"?></b></font></td>
		 		<td onclick="<?php echo $onclick_elegir?>" bgcolor="<?php echo $color_1?>" title="<?php echo $title_1?>"><?php echo $res_comprobante->fields['nombre']?></td>
		 		<td onclick="<?php echo $onclick_elegir?>"><?if ($res_comprobante->fields['nombre_medico']!="") echo $res_comprobante->fields['nombre_medico']; else echo "&nbsp"?></td>
		 		<td onclick="<?php echo $onclick_elegir?>"><?if ($res_comprobante->fields['comentario']!="") echo $res_comprobante->fields['comentario']; else echo "&nbsp"?></td>
		 		<td onclick="<?php echo $onclick_elegir?>"><?php echo fecha($res_comprobante->fields['fecha_comprobante'])?> Edad: <?php echo $res_comprobante->fields['fecha_comprobante']-$afifechanac?></td>		 		
		 		<td onclick="<?php echo $onclick_elegir?>"><?php echo "Total: ".$cant_prestaciones?></td>		 		
		 		<td onclick="<?php echo $onclick_elegir?>"><?php echo $res_comprobante->fields['periodo']?></td>		 		
		 		<td onclick="<?php echo $onclick_marcar?>" align="center"><?if ($res_comprobante->fields['marca']==1){echo "<img src='../../imagenes/salir.gif' style='cursor:pointer;'>";}
		                           ...
		                           <tr id=ma>		                               
		                            ...
		                            <?while (!$result_items1->EOF){?>
							           					<tr>
							                                 ...
							                                 <td class="bordes" title="<?php echo $result_items1->fields["codigo"]?>"><?php echo substr($result_items1->fields["codigo"],41,2)."-".substr($result_items1->fields["codigo"],43,4)."-".substr($result_items1->fields["codigo"],47,3)?></td>
							                                 <td class="bordes" title="<?php echo $descripcion_amp?>"><?php echo $descripcion?></td>			                                 
							                                 <td class="bordes"><?php echo number_format($result_items1->fields["precio"],2,',','.')?></td>
							                                 <td class="bordes"><?php echo number_format($result_items1->fields["precio"],2,',','.')?></td>
							                            </tr>
	...
	<table align='center' border=1 bordercolor='#000000' bgcolor='#FFFFFF' width='80%' cellspacing=0 cellpadding=0>
	 ...
	 <td width=30% bordercolor='#FFFFFF'>
      <table border=1 bordercolor='#FFFFFF' cellspacing=0 cellpadding=0 width=100%>
       ...
       <tr>
        <td width=30 bgcolor='#F49DD6' bordercolor='#000000' height=30>&nbsp;</td>
        <td bordercolor='#FFFFFF'>Prestaciones de COVID-19</td>
       </tr>           
      </table>
     </td>
